...
変更履歴　:
*******************************************************************/
create or replace function sp_awaf00803g_10(logkbn_in integer, status_in character varying, 
 ...
 (セッションシーケンス bigint,
  ...
  登録日時 text,
  バッチ_バッチログシーケンス bigint,
  バッチ_セッションシーケンス bigint,
  "バッチ_処理日時（開始）" text,
  "バッチ_処理日時（終了）" text,
  バッチ_メッセージ character varying,
  バッチ_パラメータ character varying,
  "バッチ_登録ユーザーID" character varying,
  バッチ_登録日時 text)
 language plpgsql
as $function$
declare
...
begin
    ...
    select 
        ...
        ,t.batchlogseq as バッチ_バッチログシーケンス
        ,t.seq2 as バッチ_セッションシーケンス
        ,to_char(t.syoridttmf2, 'YYYY/MM/DD HH24:MI:SS') as "バッチ_処理日時（開始）"
		,to_char(t.syoridttmt2, 'YYYY/MM/DD HH24:MI:SS') as "バッチ_処理日時（終了）"
        ,t.msg as バッチ_メッセージ
        ,t.pram as バッチ_パラメータ
	    ,t.reguserid2 as "バッチ_登録ユーザーID"
        ,to_char(t.regdttm2, 'YYYY/MM/DD HH24:MI:SS') as バッチ_登録日時
    from (
        select
        ...
        inner join tt_afbatchlog t9                 --バッチログテーブル
