...
変更履歴　:
*******************************************************************/
create or replace function sp_awaf00803g_11(logkbn_in integer, status_in character varying, 
 ...
 (セッションシーケンス bigint,
  ...
  登録日時 text,
  連携_外部連携ログシーケンス bigint,
  連携_セッションシーケンス bigint,
  "連携_処理日時（開始）" text,
  "連携_処理日時（終了）" text,
  連携_メッセージ character varying,
  "連携_IPアドレス（実行）" character varying,
  連携_連携区分 text,
  連携_連携方式 text,
  連携_処理区分 text,
  "連携_API連携データ" character varying,
  連携_ファイル名 character varying,
  連携_ファイルタイプ text,
  連携_ファイルサイズ integer,
  "連携_登録ユーザーID" character varying,
  連携_登録日時 text)
 language plpgsql
as $function$
declare
...
begin
    ...
    select 
        ...
        ,t.gaibulogseq as 連携_外部連携ログシーケンス
        ,t.seq2 as 連携_セッションシーケンス
        ,to_char(t.syoridttmf2, 'YYYY/MM/DD HH24:MI:SS') as "連携_処理日時（開始）"
		,to_char(t.syoridttmt2, 'YYYY/MM/DD HH24:MI:SS') as "連携_処理日時（終了）"
        ,t.msg as 連携_メッセージ
        ,t.ipadrs as "連携_IPアドレス（実行）"
        ,t.kbn || ' : ' || t.kbnnm as 連携_連携区分
        ,t.kbn2 || ' : ' || t.kbn2nm as 連携_連携方式
        ,t.kbn3 || ' : ' || t.kbn3nm as 連携_処理区分
        ,t.apidata as "連携_API連携データ"
        ,t.filenm as 連携_ファイル名
        ,t.filetype || ' : ' || t.filetypenm as 連携_ファイルタイプ
        ,t.filesize as 連携_ファイルサイズ
	    ,t.reguserid2 as "連携_登録ユーザーID"
        ,to_char(t.regdttm2, 'YYYY/MM/DD HH24:MI:SS') as 連携_登録日時
    from (
        select
        ...
        inner join tt_afgaibulog t9                 --外部連携処理結果履歴テーブル
            on t0.sessionseq = t9.sessionseq
        left join
            (select nmcd, nm 
                from tm_afmeisyo
                where nmmaincd = '1000'
                and nmsubcd = 39) m2                --名称マスタ
                on t9.kbn = m2.nmcd
        left join
            (select nmcd, nm 
                from tm_afmeisyo
                where nmmaincd = '1000'
                and nmsubcd = 30) m3                --名称マスタ
                on t9.kbn2 = m3.nmcd
        left join
            (select nmcd, nm 
                from tm_afmeisyo
                where nmmaincd = '1000'
                and nmsubcd = 7) m4                 --名称マスタ
                on t9.kbn3 = m4.nmcd
        left join
            (select nmcd, nm 
                from tm_afmeisyo
                where nmmaincd = '1000'
                and nmsubcd = 10) m5                --名称マスタ
                on cast(t9.filetype as varchar) = m5.nmcd
        where 
