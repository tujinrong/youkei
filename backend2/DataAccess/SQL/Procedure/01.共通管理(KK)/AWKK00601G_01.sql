...
*******************************************************************/
/*==============================head==============================*/
create or replace function sp_awkk00601g_01_head(gyoumukbn_in character varying, impnm_in character varying)
	returns integer
	language plpgsql
as $function$
...
$function$
/*==============================body==============================*/
create or replace function sp_awkk00601g_01_body(gyoumukbn_in character varying, impnm_in character varying, orderby_in integer, limit_in integer, offset_in integer)
	returns table
...
begin
	...
	where
		...
		(impnm_in is null or t1.torinyuryokunm like '%' || impnm_in || '%')	--一括取込入力名
	order by
		case when orderby_in = 0 then t1.gyomukbn  end				--業務区分	昇順（デフォルト）
		,case when orderby_in = 0 then t1.torinyuryokbn  end		--一括取込入力区分	昇順（デフォルト）
