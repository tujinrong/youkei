    ...
    public class Service : CmServiceBase
    {
        ...
        public InitChoiceResponse InitChoice(InitChoiceRequest req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                //-------------------------------------------------------------
                //拡事業種類データ取得
                var list = DaNameService.GetNameList(db, EnumNmKbn.拡事業種類);

                //-------------------------------------------------------------
                //２.データ加工処理
                //-------------------------------------------------------------
                //業務区分ドロップダウンリスト
                res.selectorlist1 = DaNameService.GetSelectorList(db, EnumNmKbn.拡事業業務, req.kbn1);
                //種類区分ドロップダウンリスト
        ...
        public InitKensinListResponse InitKensinJigyoList(DaRequestBase req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                //拡事業上限件数
                MAX = DaControlService.GetRow(db, EnumCtrlKbn.拡事業関連上限件数, コントロールマスタ.システム.拡事業関連上限件数._01).IntValue1;
                ...
                //停止していない拡事業一覧
                var dtl3 = dtl2.Where(e => e.kihonkakutyokbn == Enum事業区分.市区町村拡事業 && keyList2.Contains(e.jigyocd)).ToList();
                //拡事業一次件数
                var ct = dtl3.Count();
                //拡事業精密件数
                ...
                //拡事業入力件数カウント
                res.kensucnt = $"市区町村拡事業（一次{ct}/{MAX}、精密{ct2}/{MAX}）";
        ...
        public InitKensinItemListResponse InitKensinItemList(InitKensinCommonRequest req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                //拡事業上限件数
                var maxList = DaControlService.GetList(db, EnumCtrlKbn.拡事業関連上限件数);
                ...
                //半角タイプ入力項目の上限件数
                var halfMax = maxList.Find(e => e.ctrlcd == コントロールマスタ.システム.拡事業関連上限件数._02)!.IntValue1;
                //日付タイプ入力項目の上限件数
                var dayMax = maxList.Find(e => e.ctrlcd == コントロールマスタ.システム.拡事業関連上限件数._03)!.IntValue1;
                //全角タイプ入力項目の上限件数
                var fullMax = maxList.Find(e => e.ctrlcd == コントロールマスタ.システム.拡事業関連上限件数._04)!.IntValue1;
                //コードタイプ入力項目の上限件数
                var cdMax = maxList.Find(e => e.ctrlcd == コントロールマスタ.システム.拡事業関連上限件数._05)!.IntValue1;
                //半角タイプ入力項目の件数
                ...
                //拡事業入力件数カウント（一次）
                res.kensuichijicnt = $"市区町村拡項目（一次検診）（半角{halfCt1}/{halfMax}、日付{dayCt1}/{dayMax}、全角{fullCt1}/{fullMax}、コード{cdAndkensahohoCt1}/{cdMax}）";
                //拡事業入力件数カウント（精密）
                res.kensuseimitsucnt = $"市区町村拡項目（精密検査）（半角{halfCt2}/{halfMax}、日付{dayCt2}/{dayMax}、全角{fullCt2}/{fullMax}、コード{cdCt2}/{cdMax}）";
        ...
        public DaResponseBase SaveKensinItemDetail(SaveKensinItemDetailRequest req)
        {
            return Transction(req, (DaDbContext db) =>
            {
                ...
                //拡項目リスト取得
                var ctList = dtl1.Where(e => e.itemkbn != Enumフリー項目区分区分.PKG標準項目拡領域不使用).ToList();
        ...
        public InitSidoItemListResponse InitSidoItemList(DaRequestBase req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                //業務区分ドロップダウンリスト
                res.selectorlist2 = DaNameService.GetSelectorList(db, EnumNmKbn.拡_予約_保健指導業務区分,
                                                    Enum名称区分.名称).Where(e => CInt(e.value) != CInt(EnumToStr(Enum拡予約指導業務区分.予防接種))).ToList();
                //申込結果ドロップダウンリスト
        ...
        public SearchSidoItemListResponse SearchSidoItemList(SidoCommonRequest req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                //拡事業入力件数カウント
                res.kensucnt = $"市区町村拡項目（半角{halfCt}/{halfMax}、日付{dayCt}/{dayMax}、全角{fullCt}/{fullMax}、コード{cdCt}/{cdMax}）";
        ...
        private static bool IsExistTable(DaDbContext db, YoyakuSidoJigyoRowVM vm, List<tm_kksonotasidojigyoDto> dtl)
        {
            ...
            if (kbn == Enum拡予約指導業務区分.予防接種) return false;
