    ...
    public class Service : CmServiceBase
    {
        ...
        public LoginResponse Login(LoginRequest req)
        {
            ...
            //{
                ...
                //-------------------------------------------------------------
                //ユーザーマスタ
                string oracleConnectionStr = DaGlobal.OracleConnectionString;
                var oracleHelper = new OracleDbHelper(oracleConnectionStr);
                string result = oracleHelper.gj0000_cmdLogin_Click(req.userid, req.pword.ToUpper());
                string[] loginResult = result.Split('|');
                if (loginResult[0].ToString().Equals("OK") == false) {
                    res.SetGjsServiceError(loginResult[2].ToString(), loginResult[3].ToString());
                } 
                else
                {
                    res.userinfo = new UserInfoVM();
                    res.userinfo.usernm = loginResult[1].ToString();
                    res.userinfo.pSiyoKbn = loginResult[2].ToString();
                }
                ...
                ////-------------------------------------------------------------
                ////エラーがある場合、エラー回数＋1
                //if (res.returncode != EnumServiceResult.OK.ToString)
                //{
                //    dto.errorkaisu += 1;
                //    //ユーザーマスタ
