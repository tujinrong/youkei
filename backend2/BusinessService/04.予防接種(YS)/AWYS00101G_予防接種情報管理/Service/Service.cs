...
namespace BCC.Affect.Service.AWYS00101G
{
    ...
    public class Service : CmServiceBase
    {
        ...
        public CommonResponse SaveSessyu(SaveSessyuRequest req)
        {
        ...
        public SearchDetailResponse SearchDetail(DaDbContext db, SearchDetailRequest req)
        {
            ...
            //C、予防接種情報一覧を取得する
            res.kekkalist = GetKekkaList(db, req.atenano, EnumHanyoKbn.接種種類詳細コード, req.sessyufilterkbn, req.rirekihyoji, (int)Enum生涯１回フラグ.生涯1回, req.pagenum, req.pagesize, req.orderby);
        ...
        public SearchDetailMultiResponse SearchDetailMulti(DaDbContext db, SearchDetailMultiRequest req)
        {
            ...
            //-------------------------------------------------------------
            //B、予防接種情報一覧を取得する
            res.kekkalist = GetKekkaList(db, req.atenano, EnumHanyoKbn.接種種類詳細コード, req.sessyufilterkbn, true, (int)Enum生涯１回フラグ.生涯複数回, req.pagenum, req.pagesize, req.orderby);
        ...
        public SearchDetailOtherResponse SearchDetailOther(DaDbContext db, SearchDetailOtherRequest req)
        {
            ...
            //C、予防接種依頼（フリー項目）コントロールマスタ情報を取得する
            //TODO：予防接種用（フリー項目）コントロールマスタは一つテーブルできるはずなので将来テーブル名変わる可能性が高い
            ...
            //画面項目設定
            //集約コードパターン
            var patternno = CmLogicUtil.GetPatternno(AWYS00101G, 拡_予約_保健指導業務区分._03);
        ...
        private SearchSessyuDetailResponse SearchSessyuDetail(DaDbContext db, SearchSessyuDetailRequest req)
        {
            ...
            //B．予防接種テーブルから該当宛名番号指定した接種種類コード／回数／枝番の情報を取得する
            ...
            //編集フラグ
            res.editflg = string.IsNullOrEmpty(sessyudto?.regsisyo) || sisyoList.Contains(CStr(sessyudto?.regsisyo));
        ...
        private SearchSessyuIraiDetailResponse SearchSessyuIraiDetail(DaDbContext db, SearchSessyuIraiDetailRequest req)
        {
            ...
            //編集フラグ
            res.editflg = string.IsNullOrEmpty(sessyuiraidto?.regsisyo) || sisyoList.Contains(CStr(sessyuiraidto?.regsisyo));
        ...
        /// 保存処理(接種情報詳細画面)
        /// </summary>
        private CommonResponse SaveSessyu(DaDbContext db, SaveSessyuRequest req)
        {
        ...
        ///接種種類フィルター区分りすとを取得
        /// </summary>
        public List<SessyuOneVm> GetKekkaList(DaDbContext db, string atenano, EnumHanyoKbn Hanyokbn, string sessyufilterkbn, bool rirekihyoji, int kbn, int pagenum, int pagesize, int orderby)
        {
            ...
            //A、パラメータ取得
            var param = Converter.GetParameters(atenano, maincd, subcd, sessyufilterkbn, rirekihyoji, kbn);
        ...
        /// 風疹抗体検査画面のデータ操作区分取得
        /// </summary>
        private EnumActionType? GetKBN(DaDbContext db, string atenano)
        {
            EnumActionType? kbn = null;
        ...
        private EnumActionType? GetKojinKBN(DaDbContext db, string atenano, string prockbn)
        {
            ...
            {
                if (db.tt_yskojinfree.SELECT.WHERE.ByKey(atenano).Exists())
                {
                    kbn = EnumActionType.Update;        //編集フラグ☑=>更新
                }
                else
        ...
        /// 保存前チェック（接種情報）
        /// </summary>
        private static bool Check(DaDbContext db, SaveSessyuRequest req, EnumActionType? kbn, CommonResponse res)
        {
        ...
        private static bool CheckKojinDataExist(DaDbContext db, string atenano, string prockbn)
        {
            ...
            if (prockbn.Equals(詳細画面区分._その他))
            {
                if (db.tt_yskojinfree.SELECT.WHERE.ByKey(atenano).Exists()) { return true; }
            }
            else if (prockbn.Equals(詳細画面区分._風疹抗体))
            {
