    ...
    public class Service : CmServiceBase
    {
        ...
        public InitResponse Init(DaRequestBase req)
        {
            return Nolock(req, db =>
            {
                ...
                var orderBy = new List<DaSelectorModel>();          //ドロップダウンリスト(並び替え)
                orderBy.Add(new DaSelectorModel(((int)Enum並び替え.無し).ToString(), Enum並び替え.無し.ToString()));
                orderBy.Add(new DaSelectorModel(((int)Enum並び替え.昇順).ToString(), Enum並び替え.昇順.ToString()));
                orderBy.Add(new DaSelectorModel(((int)Enum並び替え.降順).ToString(), Enum並び替え.降順.ToString()));
                res.selectorlist3 = orderBy;                        //(並び替え)
