    ...
    public class Service : CmServiceBase
    {
        ...
        public InitDetailResponse InitDetail(InitDetailRequest req)
        {
            return Nolock(req, db =>
            {
                ...
                //プロシージャ提示用リスト
                res.parameterList = DaSystemParameterManager.GetparameterValues();
                return res;
            });
        }
        ...
        public InitItemsTabResponse SearchItemsTab(YosikiRequestTabBase req)
        {
            return Nolock(req, db =>
            {
                ...
                //項目大分類
                Dictionary<string, string>? itemId2DaiBunruiDict = null;
        ...
        public DaResponseBase SaveProject(SaveProjectRequest req)
        {
            return Transction(req, db =>
            {
                ...
                {
                    //帳票IDの存在チェック
                    if (req.kbn == Enum帳票様式.帳票 && db.tm_eurpt.SELECT.WHERE.ByKey(req.rptid).Exists())
                    {
                ...
                //項目定義の出力順詳細リスト
                if (req.sortDetailParam != null)
                {
                    var sortptnno = SaveSortLineParam(db, req.sortDetailParam, req.rptid, req.yosikiid);
                    euyosikiDto.sortptnno = sortptnno;
                }
        ...
        private void SetSelectorList(DaDbContext db, InitDetailResponse res, string rptid)
        {
            ...
            //ドロップダウンリスト(帳票発行履歴区分)
            res.hakokbnList = GetHanyoSelectorList(db, EnumHanyoKbn.帳票発行履歴区分);
        ...
        //基本の並び替え設定
        private const string PROC_SORTNAME = "基本の並び替え設定";
        //並び替え設定(項目定義タブ)
        private int SaveSortLineParam(DaDbContext db, SortLineParam sortParam, string rptid, string yosikiid)
        {
            //-------------------------------------------------------------
            //１.データ取得
            //-------------------------------------------------------------   
            var filter = $"{nameof(tt_eusortDto.rptid)} = @{nameof(tt_eusortDto.rptid)} " +
            ...
            var eusortDtl = db.tt_eusort.SELECT.WHERE.ByFilter(filter, rptid, yosikiid, PROC_SORTNAME).GetDto();
            int sortptnno = 0;
            if (eusortDtl == null)
            {
                sortptnno = db.tt_eusort.SELECT.WHERE.ByKey(rptid, yosikiid).GetMax<int>(nameof(tt_eusortDto.sortptnno)) + 1;
                var dto = new tt_eusortDto();
                dto.rptid = rptid;                           //帳票ID
                dto.yosikiid = yosikiid;                     //様式ID
                dto.sortptnnm = PROC_SORTNAME;               //出力順パターン名
                dto.sortptnno = sortptnno;                   //出力順パターン番号
                var sortSubDtl = Converter.GetAddSortSubDtl(rptid, yosikiid, sortParam.sortsublist, sortptnno);
                //出力順パターンテーブル登録
                db.tt_eusort.INSERT.Execute(dto);
                //出力順パターンサブテーブル登録
                db.tt_eusort_sub.INSERT.Execute(sortSubDtl);
            }
            else
            {
                sortptnno = sortParam.sortptnno;             //出力順パターン番号
                //出力順パターンサブテーブル削除
                db.tt_eusort_sub.DELETE.WHERE.ByKey(rptid, yosikiid, sortptnno).Execute();
                var sortSubDtl = Converter.GetAddSortSubDtl(rptid, yosikiid, sortParam.sortsublist, sortParam.sortptnno);
                //出力順パターンサブテーブル登録
                db.tt_eusort_sub.INSERT.Execute(sortSubDtl);
            }
            //正常返し
