...
namespace BCC.Affect.Service.AWEU00201G
{
    public class Converter : CmConerterBase
    {
        ...
        public static tm_eurptDto GetReportDto(tm_eurptDto eureportDto, SaveProjectRequest req)
        {
            ...
            eureportDto.ninsanpuflg = req.rptDetailParam.ninsanpuflg;                   //妊産婦フラグ
            ...
            if (startIndex != -1 && endIndex != -1 && endIndex > startIndex)
            {
                ...
                for (int i = 0; i < functionVO!.conditionparams.Count && i < valuesList.Count; i++)
                {
                    ...
                    if (!parameterValue!.Contains("@"))
                    {
                        ...
                        if (parameterValue != null && DaEucService.IsParameterValid(cell.DataType, parameterValue))
                        {
                            continue;
                        }
                        else
                        {
                            errorMeg = DaMessageService.GetMsg(EnumMessage.ITEM_NOTEQUAL_ERROR, $"{sqlName}の{parameterValue}のデータ型");
                            return errorMeg;
                        }
                    }
                }
        ...
        public static List<tm_eurptkensakuDto> GeReportCoditionDtl(List<SearchConditionVM> rptConditionList, string rptid, Enum様式作成方法 yosikihouhou)
        {
            ...
            //}
            return rptConditionList.Select(x => GetReportCoditionDto(x, rptid, yosikihouhou)).ToList();
        }

        /// <summary>
        /// 帳票の検索条件を取得
        /// </summary>
        private static tm_eurptkensakuDto GetReportCoditionDto(SearchConditionVM vm, string rptid, Enum様式作成方法 yosikihouhou)
        {
            var dto = new tm_eurptkensakuDto();
            dto.rptid = rptid;                                      //帳票ID
            dto.datasourceid = vm.datasourceid;                     //データソースID
