    ...
    Public Class Service
        ...
        Public Function Login(req As LoginRequest) As LoginResponse
            Return Nolock(req, Function(db)
                ...
                Try
                    '検索結果出力用ＳＱＬ作成 
                    If Not f_Search_SQLMake(sSql, req.userid, res) Then
                        Exit Try
                    End If
                    ...
                    If dstDataSet.Tables(0).Rows.Count > 0 Then
                        '画面に該当データを表示
                        '画面にセット
                        If Not f_User_Check(dstDataSet, req.pword, req.userid, res) Then
                            Exit Try
                        End If

                        'トークンの設定
                        Dim loginTime = DateTime.Now.ToString("yyyyMMddHHmmss")
                        Dim tokenId = req.userid & "|" & loginTime
        ...
        Public Function GetUserInfo(req As LoginRequest) As UserInfoResponse
            Return Nolock(req, Function(db)
