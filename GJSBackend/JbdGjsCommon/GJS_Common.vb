...
'*　　②　機能概要　　　　　ＥＭＳ共通モジュール
'*
'*  　③　更新履歴　　　　　2014/12/19 JBD368　GJS開発では共通モジュールはGJS_Commonのみを参照する
'*******************************************************************************
Option Strict Off
Option Explicit On
...
#Region "*** 変数定義 ***"
    ...
    ' Public pJIG_CD As String
    ' Public pTANTO As String
    ...
    'Public myCSV_SANKA_ENTRY As String
    'Public myCSV_SANKA_ENTRY_SERVER As String
    'Public myCSV_KANRI_DATA_SERVER As String
    'Public myTXT_KANRI_ENTRY As String
    'Public myTXT_KANRI_ENTRY_SERVER As String
    'Public myDMP_FILE_PATH As String
    'Public myDMP_LOG_PATH As String
    'Public myDMP_TIME_OUT As String
    'Public myDMP_DBUSER As String
    'Public myDMP_DBPASS As String
    'Public myDMP_DBSID As String
    ...
    '2014/12/24 JBD368 UPD
    'Public myBACKCOLOR_STRING As String = "AliceBlue"   '2014/05/09　画面背景色
    Public myBACKCOLOR_STRING As String = String.Empty
    '2014/12/24 JBD368 UPD"
    Public myBACKCOLOR As System.Drawing.Color          '2014/05/09　画面背景色


    '2008/12/01▽振込データテキスト出力処理で追加*********************************************************************************
    Public myTXT_FURIKOMI_ENTRY As String
    '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
    'Public myTXT_FURIKOMI_ENTRY_SERVER As String
    ''2008/12/01△振込データテキスト出力処理で追加*********************************************************************************
    ''2010/10/13▽引落データテキスト出力処理で追加*********************************************************************************
    'Public myTXT_HIKIOTOSHI_ENTRY As String
    ''2010/10/13△引落データテキスト出力処理で追加*********************************************************************************
    ''2008/12/26▽PDFデータ圧縮・退避処理で追加*********************************************************************************
    'Public myZIP_PASSWORD As String
    'Public myZIP_PATH As String
    'Public myBACKUP_PATH As String
    ''2008/12/26▽PDFデータ圧縮・退避処理で追加*********************************************************************************
    '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
    ...
    Public Structure pGJSINI
        ...
        '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
        'Public CSV_SANKA_ENTRY As String
        'Public CSV_SANKA_ENTRY_SERVER As String
        'Public CSV_KANRI_DATA_SERVER As String
        'Public TXT_KANRI_ENTRY As String
        'Public TXT_KANRI_ENTRY_SERVER As String
        '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
        Public TXT_FURIKOMI_ENTRY As String
        '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
        'Public TXT_FURIKOMI_ENTRY_SERVER As String
        'Public TXT_HIKIOTOSHI_ENTRY As String
        'Public DMP_FILE_PATH As String
        'Public DMP_LOG_PATH As String
        'Public DMP_DBUSER As String
        'Public DMP_DBPASS As String
        'Public DMP_DBSID As String
        'Public DMP_TIME_OUT As String
        '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
    ...
    Private xl As New JbdGjsCommon.Excel
...
#Region "*** 初期処理関連 ***"
    ...
    '------------------------------------------------------------------
    Public Function FormInit() As Boolean
        ' Declare variables.
        ...
        Try
            ...
            ' コネクション文字列の設定
            '-------------------------------------------------------
            'Cnn.ConnectionString = "user id=sin;password=sin;data source=orcl"
    ...
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Public Function Get_IniFile() As Boolean
        Dim ret As Integer
        Get_IniFile = False
        Try

            iniFileName = "GJS.ini"  'INIファイル名（直接フルパスを指定してもOK）
            ...
            'INIファイルよりキーの値を読み込み
            ret = GetPrivateProfileString("common", "DBUSER", "", _
                                          strBuffer, strBuffer.Capacity, iniFileName)
            myDBUSER = strBuffer.ToString

            ret = GetPrivateProfileString("common", "DBPASS", "", _
                                          strBuffer, strBuffer.Capacity, iniFileName)
            myDBPASS = strBuffer.ToString

            ret = GetPrivateProfileString("common", "DBSID", "", _
                                          strBuffer, strBuffer.Capacity, iniFileName)
            myDBSID = strBuffer.ToString

            '2014/05/09　追加開始
            ret = GetPrivateProfileString("common", "BACKCOLOR", "", _
                                          strBuffer, strBuffer.Capacity, iniFileName)
            myBACKCOLOR_STRING = strBuffer.ToString
            myBACKCOLOR = f_ColorConvert(myBACKCOLOR_STRING)
            '2014/05/09　追加終了

            '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
            'ret = GetPrivateProfileString("csv", "SANKA_ENTRY", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myCSV_SANKA_ENTRY = strBuffer.ToString


            'ret = GetPrivateProfileString("csv", "SANKA_ENTRY_SERVER", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myCSV_SANKA_ENTRY_SERVER = strBuffer.ToString

            'ret = GetPrivateProfileString("csv", "KANRI_DATA_SERVER", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myCSV_KANRI_DATA_SERVER = strBuffer.ToString

            'ret = GetPrivateProfileString("txt", "KANRI_TXT_ENTRY", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myTXT_KANRI_ENTRY = strBuffer.ToString

            'ret = GetPrivateProfileString("txt", "KANRI_TXT_ENTRY_SERVER", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myTXT_KANRI_ENTRY_SERVER = strBuffer.ToString
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
            '2008/12/01▽振込データテキスト出力処理で追加*********************************************************************************
            ret = GetPrivateProfileString("txt", "FURIKOMI_TXT_ENTRY", "", _
                                          strBuffer, strBuffer.Capacity, iniFileName)
            myTXT_FURIKOMI_ENTRY = strBuffer.ToString
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
            'ret = GetPrivateProfileString("txt", "FURIKOMI_TXT_ENTRY_SERVER", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myTXT_FURIKOMI_ENTRY_SERVER = strBuffer.ToString
            ''2008/12/01△振込データテキスト出力処理で追加*********************************************************************************
            ''2010/10/13▽引落データテキスト出力処理で追加*********************************************************************************
            'ret = GetPrivateProfileString("txt", "HIKIOTOSHI_TXT_ENTRY", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myTXT_HIKIOTOSHI_ENTRY = strBuffer.ToString
            ''2010/10/13△引落データテキスト出力処理で追加*********************************************************************************
            'ret = GetPrivateProfileString("dmp", "DMP_FILE_PATH", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_FILE_PATH = strBuffer.ToString

            'ret = GetPrivateProfileString("dmp", "DMP_LOG_PATH", "", _
            '                  strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_LOG_PATH = strBuffer.ToString

            'ret = GetPrivateProfileString("dmp", "DMP_DBUSER", "", _
            '                  strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_DBUSER = strBuffer.ToString

            'ret = GetPrivateProfileString("dmp", "DMP_DBPASS", "", _
            '                  strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_DBPASS = strBuffer.ToString

            'ret = GetPrivateProfileString("dmp", "DMP_DBSID", "", _
            '                  strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_DBSID = strBuffer.ToString

            'ret = GetPrivateProfileString("dmp", "DMP_TIME_OUT", "", _
            '                  strBuffer, strBuffer.Capacity, iniFileName)
            'myDMP_TIME_OUT = strBuffer.ToString
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
            ret = GetPrivateProfileString("report", "PDF_PATH", "", _
                              strBuffer, strBuffer.Capacity, iniFileName)
            myREPORT_PDF_PATH = strBuffer.ToString

            ret = GetPrivateProfileString("report", "EXCEL_PATH", "", _
                              strBuffer, strBuffer.Capacity, iniFileName)
            myREPORT_EXCEL_PATH = strBuffer.ToString

            '2015/03/23 ADD SATRT
            ret = GetPrivateProfileString("report", "TEMPLATE_EXCEL_PATH", "", _
                              strBuffer, strBuffer.Capacity, iniFileName)
            myTEMPLATE_EXCEL_PATH = strBuffer.ToString
            '2015/03/23 ADD END
            ...
            ''2008/12/26▽PDFデータ圧縮・退避処理で追加*********************************************************************************
            'ret = GetPrivateProfileString("zip", "ZIP_PASSWORD", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myZIP_PASSWORD = strBuffer.ToString

            'ret = GetPrivateProfileString("zip", "ZIP_PATH", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myZIP_PATH = strBuffer.ToString

            'ret = GetPrivateProfileString("backup", "BACKUP_PATH", "", _
            '                              strBuffer, strBuffer.Capacity, iniFileName)
            'myBACKUP_PATH = strBuffer.ToString
            ''2008/12/26△PDFデータ圧縮・退避処理で追加*********************************************************************************
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
            '帳票余白初期値設定
            myYOHAKU_UP = C_YOHAKU_UP               '帳票上余白
            myYOHAKU_DOWN = C_YOHAKU_DOWN           '帳票下余白
            myYOHAKU_LEFT = C_YOHAKU_LEFT           '帳票左余白
            myYOHAKU_RIGHT = C_YOHAKU_RIGHT         '帳票右余白

            myANNAI_CD = 0                          '2010/11/08 ADD JBD200 案内文マスタの種類初期表示用
            myTAISYO_NENGETU = Nothing              '2012/02/21 ADD JBD200 承認法人受入履歴用
    ...
    Public Function DbConnect() As Boolean
        ...
        Try
            ...
            If Cnn Is Nothing Or Cnn.ConnectionString = "" Then
                pConnection = ""
                pConnection = pConnection & "user id=" & myDBUSER
                pConnection = pConnection & ";password=" & myDBPASS
                pConnection = pConnection & ";data source=" & myDBSID
                'コネクション文字列の設定
    ...
    Public Sub ACoSubFormControlBoxSet(ByVal objForm As Form)
        Dim hMenu As Integer
        Call GetSystemMenu(objForm.Handle.ToInt32, 1)
        hMenu = GetSystemMenu(objForm.Handle.ToInt32, 0)
        Call EnableMenuItem(hMenu, &HF060, &H0S Or &H1S)

    End Sub

    '------------------------------------------------------------------
    'プロシージャ名  :Set_CommonInfo
    '説明            :サブ画面用共通情報取得
    '戻り値          :なし
    '------------------------------------------------------------------
    Public Sub Set_CommonInfo()

        '構造体にOracle接続用情報がセットされていない場合は、ini情報の取得を行う
        If myDBUSER = "" And myDBPASS = "" And myDBSID = "" Then
            If Not Get_IniFile() Then
                Exit Sub
            End If
        End If
    ...
    '------------------------------------------------------------------
    'プロシージャ名  :Set_CommonInfo
    '説明            :GJS.INI構造体の内容をグローバル変数に取得
    '戻り値          :なし
    '------------------------------------------------------------------
    Public Sub Set_CommonInfo(ByVal pGJSINI_Array As pGJSINI)

        '構造体にOracle接続用情報がセットされていない場合は、ini情報の取得を行う
        If pGJSINI_Array.DBUSER = "" And pGJSINI_Array.DBPASS = "" And pGJSINI_Array.DBSID = "" Then
            If Not Get_IniFile() Then
                Exit Sub
            End If
        Else
            '引数で引き渡されたGJS.INI構造体の内容をグローバル変数へセット
            pLOGINUSERID = pGJSINI_Array.LOGINUSERID                                    'ログインユーザーＩＤ  2010/10/23 ADD JBD200
            pLOGINUSERNM = pGJSINI_Array.LOGINUSERNM                                    'ログインユーザー名
            myDBUSER = pGJSINI_Array.DBUSER                                             'Oracle接続用USER
            myDBPASS = pGJSINI_Array.DBPASS                                             'Oracle接続用PW
            myDBSID = pGJSINI_Array.DBSID                                               'Oracle接続用SID
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
            'myCSV_SANKA_ENTRY = pGJSINI_Array.CSV_SANKA_ENTRY                           '
            'myCSV_SANKA_ENTRY_SERVER = pGJSINI_Array.CSV_SANKA_ENTRY_SERVER             '
            'myCSV_KANRI_DATA_SERVER = pGJSINI_Array.CSV_KANRI_DATA_SERVER               '
            'myTXT_KANRI_ENTRY = pGJSINI_Array.TXT_KANRI_ENTRY                           '
            'myTXT_KANRI_ENTRY_SERVER = pGJSINI_Array.TXT_KANRI_ENTRY_SERVER             '
            'myDMP_FILE_PATH = pGJSINI_Array.DMP_FILE_PATH                               '
            'myDMP_LOG_PATH = pGJSINI_Array.DMP_LOG_PATH                                 '
            'myDMP_TIME_OUT = pGJSINI_Array.DMP_TIME_OUT                                 '
            'myDMP_DBUSER = pGJSINI_Array.DMP_DBUSER                                     '
            'myDMP_DBPASS = pGJSINI_Array.DMP_DBPASS                                     '
            'myDMP_DBSID = pGJSINI_Array.DMP_DBSID                                       '
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
            myREPORT_PDF_PATH = pGJSINI_Array.REPORT_PDF_PATH                           'PDF出力PATH
            myREPORT_EXCEL_PATH = pGJSINI_Array.REPORT_EXCEL_PATH                       'EXCEL出力PATH
            '2015/03/23 ADD START
            myTEMPLATE_EXCEL_PATH = pGJSINI_Array.TEMPLATE_EXCEL_PATH                   'EXCELテンプレート用PATH
            '2015/03/23 ADD END
            myTXT_FURIKOMI_ENTRY = pGJSINI_Array.TXT_FURIKOMI_ENTRY                     '
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL START
            'myTXT_FURIKOMI_ENTRY_SERVER = pGJSINI_Array.TXT_FURIKOMI_ENTRY_SERVER       '
            'myTXT_HIKIOTOSHI_ENTRY = pGJSINI_Array.TXT_HIKIOTOSHI_ENTRY                 '引落ＣＳＶファイルＰＡＴＨ  2010/10/13 ADD
            'myZIP_PASSWORD = pGJSINI_Array.ZIP_PASSWORD                                 '
            'myZIP_PATH = pGJSINI_Array.ZIP_PATH                                         '
            'myBACKUP_PATH = pGJSINI_Array.BACKUP_PATH                                   '
            '2021/04/12 JBD9020 R03年度追加基金対応 DEL END
            myREPORT_PDF_OUTKBN = pGJSINI_Array.REPORT_PDF_OUTKBN                       'PDF自動出力
            myANNAI_CD = pGJSINI_Array.ANNAI_CD                                         '2010/11/08 ADD JBD200 案内文マスタの種類初期表示用
            myTAISYO_NENGETU = pGJSINI_Array.TAISYO_NENGETU                             '2012/02/21 ADD JBD200 承認法人受入履歴用

            myYOHAKU_UP = pGJSINI_Array.YOHAKU_UP                                       '帳票上余白
            myYOHAKU_DOWN = pGJSINI_Array.YOHAKU_DOWN                                   '帳票下余白
            myYOHAKU_LEFT = pGJSINI_Array.YOHAKU_LEFT                                   '帳票左余白
            myYOHAKU_RIGHT = pGJSINI_Array.YOHAKU_RIGHT                                 '帳票右余白

            myBACKCOLOR_STRING = pGJSINI_Array.SCR_BACKCOLOR_STRING                     '背景色    2014/05/09　追加
            myBACKCOLOR = pGJSINI_Array.SCR_BACKCOLOR                                   '背景色    2014/05/09　追加
...
#Region "*** 日付制御関数 ***"
    Public Function ACoDateCheckEdit(ByVal objTextBox As TextBox, _
        ...
        Try
            ...
            If Len(objTextBox.Text) = 8 And Not (Mid$(objTextBox.Text, 3, 1) = "-" Or Mid$(objTextBox.Text, 3, 1) = "/") Then
                ...
                If Mid$(objTextBox.Text, 5, 2) < "01" Or Mid$(objTextBox.Text, 5, 2) > "12" Or _
            ...
            If Len(objTextBox.Text) = 8 And (Mid$(objTextBox.Text, 3, 1) = "-" Or Mid$(objTextBox.Text, 3, 1) = "/") Then
                If Left$(objTextBox.Text, 2) < "00" Or Left$(objTextBox.Text, 2) > "99" Or _
                    Mid$(objTextBox.Text, 4, 2) < "01" Or Mid$(objTextBox.Text, 4, 2) > "12" Or _
                    Mid$(objTextBox.Text, 7, 2) < "01" Or Mid$(objTextBox.Text, 7, 2) > "31" Then
                    GoTo ACoDateCheckEdit_Exit1
                End If
            ...
            If Len(objTextBox.Text) = 10 And (Mid$(objTextBox.Text, 5, 1) = "-" Or Mid$(objTextBox.Text, 5, 1) = "/") Then
                ...
                If Mid$(objTextBox.Text, 6, 2) < "01" Or Mid$(objTextBox.Text, 6, 2) > "12" Or _
...
#Region "f_Select_ODP SELECT"
    ...
    Public Function f_Select_ODP(ByRef dstDatasetSend As DataSet, ByVal strSQL As String) As Boolean
        ...
        Try
            ...
            daDataAdapter = New OracleDataAdapter(strSQL, Cnn)
...
#Region "メッセージ関連"
    ...
    '------------------------------------------------------------------
    Public Function Show_MessageBox(ByVal CD As String, _
                                    ByVal Message As String, _
                                    ByVal varBUTTON As String, _
                                    ByVal varICON As String, _
