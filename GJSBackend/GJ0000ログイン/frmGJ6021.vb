...
#Region "cmdEnd_Click 戻るボタンクリック処理"
    ...
    Private Sub cmdEnd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdEnd.Click
        Try
            ...
            'pblnTextChange = False      '画面入力内容変更フラグ初期化 'R05年度　OSバージョンアップ　既存バグ修正のため変更　データ変更のメッセージを表示しないので変更判定が不要のため削除　JBD453 2024/03/13 DEL END
            'フォームクローズ
    ...
    Private Function f_Report_Output() As Boolean
        ...
        Try
            ...
            If wkDSRep.Tables(0).Rows.Count > 0 Then
                Dim w As New rptGJ6021
                w.sub1(wkDSRep)
                'Using wkAR As New rptGJ6021
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSQL As String) As Boolean
        ...
        If rdoKizon.Checked Then
            kbn = "_1"
            head = " , '既存基金' AS KIKIN_KUBUN " & vbCrLf
        ElseIf rdoTuika.Checked Then
            kbn = "_2"
            head = " , '追加基金' AS KIKIN_KUBUN " & vbCrLf
        Else
            kbn = ""
            head = " , '総合計' AS KIKIN_KUBUN " & vbCrLf
        End If
        ...
        Try
            ...
            '返還係数
            wSQL += "  , H.HENKAN_KEISU" & kbn & " AS HENKAN_KEISU" & vbCrLf
            '契約者番号
            ...
            '当期生産者積立金納付額
            wSQL += "  , H.TUMITATE_KIN_KEI" & kbn & " AS TUMITATE_KIN_KEI" & vbCrLf
            '互助金交付額まで印字、繰越額算定・確定まで印字が選択されている場合のみ出力
            '当期互助金交付額
            If rdoKofu.Checked Or rdoKurikoshi.Checked Then
                wSQL += "  , H.KOFU_KIN_KEI" & kbn & " AS KOFU_KIN_KEI" & vbCrLf
            Else
            ...
            If rdoKurikoshi.Checked Then
                '生産者積立金返還算定額
                wSQL += "  , H.HENKAN_SANTEI_KIN" & kbn & " AS HENKAN_SANTEI_KIN" & vbCrLf
                '調整金
                wSQL += "  , H.CYOSEI_KIN" & kbn & " AS CYOSEI_KIN" & vbCrLf
                '生産者積立金返還額(確定)
                wSQL += "  , H.HENKAN_KAKUTEI_KIN" & kbn & " AS HENKAN_KAKUTEI_KIN" & vbCrLf
            Else
                '生産者積立金返還算定額
            ...
            '追加だった場合追加（_2）のデータがないときは出力しない
            If rdoTuika.Checked Then
                wSQL += "    AND H.TUMITATE_KIN_KEI_2 IS NOT NULL " & vbCrLf
            End If
...
#Region "f_ObjectClear 画面クリア処理"
    ...
    Private Function f_ObjectClear(ByVal wKbn As String) As Boolean
        ...
        Try
            ...
            '画面変更フラグ
            'pblnTextChange = False 'R05年度　OSバージョンアップ　既存バグ修正のため変更　データ変更のメッセージを表示しないので変更判定が不要のため削除　JBD453 2024/03/13 DEL END
