...
#Region "cmdPreview_Click プレビューボタンクリック時処理"
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        ...
        Try
            ...
            '採択通知書発行処理の更新確認メッセージ
            If pCurrentKey.KEIYAKU_KBN_MAE = 1 Then
                '家族→企業
    ...
    End Sub
#End Region
#Region "cmdTorikesi_Click 通知書取消ボタンクリック時処理"
    '------------------------------------------------------------------
...
#Region "f_Data_Save0 仮請求処理"
    ...
    Private Function f_Data_Save0() As Boolean
        ...
        Try
            ...
            Cmd.CommandText = "PKG_GJ3021.GJ3021_KARI_SEIKYU"
            ...
            Cmd.Parameters.Add("IN_KEIYAKU_KBN_MAE", pCurrentKey.KEIYAKU_KBN_MAE)
            '契約区分　1:家族 2:企業 3:うずら 
            Cmd.Parameters.Add("IN_KEIYAKU_KBN_ATO", pCurrentKey.KEIYAKU_KBN_ATO)
...
#Region "f_Data_Save1 初回請求処理"
    ...
    Private Function f_Data_Save1() As Boolean
        ...
        Try
            ...
            Cmd.Connection = Cnn
            Cmd.CommandType = CommandType.StoredProcedure
            Cmd.CommandText = "PKG_GJ3021.GJ3021_SYOKAI_SEIKYU"
...
#Region "f_Data_Save2 再発行処理"
    ...
    Private Function f_Data_Save2() As Boolean
        ...
        Try
            ...
            Cmd.Connection = Cnn
            Cmd.CommandType = CommandType.StoredProcedure
            Cmd.CommandText = "PKG_GJ3021.GJ3021_SAI_HAKKO"
...
#Region "f_Data_Save3 修正発行処理"
    ...
    Private Function f_Data_Save3() As Boolean
        ...
        Try
            ...
            Cmd.Connection = Cnn
            Cmd.CommandType = CommandType.StoredProcedure
            Cmd.CommandText = "PKG_GJ3021.GJ3021_SYUSEI_HAKKO"
...
#Region "f_Data_Save4 請求取消処理"
    ...
    Private Function f_Data_Save4() As Boolean
        ...
        Try
            ...
            Cmd.Connection = Cnn
            Cmd.CommandType = CommandType.StoredProcedure
            Cmd.CommandText = "PKG_GJ3021.GJ3021_SEIKYU_TORIKESI"
            '----------------------------------------
            '   情報
            '----------------------------------------
            '期   
...
#Region "f_InitSet ラベルセット処理"
    ...
    Private Function f_InitSet() As Boolean
        ...
        Try
            ...
            lbl_KEIYAKU_KBN_MAE.Text = pCurrentKey.KEIYAKU_KBN_MAE
            lbl_KEIYAKU_KBN_MAE_NM.Text = pCurrentKey.KEIYAKU_KBN_MAE_NM
            ...
            If pCurrentKey.KEIYAKU_KBN_MAE = 1 Then
                '家族→企業
                lbl_KIGEN_DATE.Text = "納付期限"
            Else
                '企業→家族
                lbl_KIGEN_DATE.Text = "振込予定日"
            End If
...
#Region "f_Input_Check_Previw 画面入力チェック処理(プレビュー)"
    ...
    Private Function f_Input_Check_Previw() As Boolean
        ...
        Try
            ...
            '消費税率マスタなしの場合、エラー
            If rbtn_SYORI_KBN0.Checked Or rbtn_SYORI_KBN1.Checked Then
...
#Region "F_DataKbnChk データ区分チェック"
    ...
    Private Function F_DataKbnChk() As Boolean
        ...
        Try
            ...
            '--------------------------------------------------
            wSql &= "SELECT"
            wSql &= "  COUNT(*) RIREKI_CNT,"
            wSql &= "  MIN( NVL(DATA_FLG, 0) ) DATA_FLG "
            wSql &= "FROM"
            wSql &= "  TT_KEIYAKU_KBN_MEISAI_RIREKI RME,"
            wSql &= "  TT_KEIYAKU_JOHO JOH "
            wSql &= "WHERE RME.KI = " & pCurrentKey.KI
            wSql &= "  AND RME.KEIYAKUSYA_CD = " & pCurrentKey.KEIYAKUSYA_CD
            ...
            wSql &= "  AND RME.MAE_SEQNO = JOH.SEQNO(+)"
            ...
            Else
                ...
                End If
                '履歴明細データ件数
                wRirekiCnt = WordHenkan("N", "Z", wkDS.Tables(0).Rows(0)("RIREKI_CNT"))
            End If
            ...
            wSql &= "FROM"
            wSql &= "  TT_KEIYAKU_JOHO JOH "
            wSql &= "WHERE JOH.KI = " & pCurrentKey.KI
            wSql &= "  AND JOH.KEIYAKUSYA_CD = " & pCurrentKey.KEIYAKUSYA_CD
            ...
            Else
                '履歴明細データ件数
                wJohoCnt = WordHenkan("N", "Z", wkDS.Tables(0).Rows(0)("JOHO_CNT"))
            End If

            '--------------------------------------------------
            '件数比較
            '--------------------------------------------------
            If wRirekiCnt <> wJohoCnt Then
                '該当データなし
                Show_MessageBox_Add("I007", "契約情報が保存時と異なるため処理できません。")
                Exit Try
            End If
...
#Region "f_SyoriKbnChk 入力中チェック処理"
    ...
    Private Function f_SyoriKbnChk() As Boolean
        ...
        Try
            ...
            wSql &= "  AND RIR.KEIYAKUSYA_CD = " & pCurrentKey.KEIYAKUSYA_CD
...
#Region "f_TankaChk 単価マスタ　存在チェック処理"
    ...
    Private Function f_TankaChk() As Boolean
        ...
        Try
            ...
            wSql &= "  AND TAN.KEIYAKU_KBN = " & pCurrentKey.KEIYAKU_KBN_ATO
    ...
    Private Function f_Report_Output() As Boolean
        ...
        Try
            ...
            If wkDSRep.Tables(0).Rows.Count > 0 Then
                ...
                Dim w As New rptGJ3021
                w.sub1(wkDSRep, pCurrentKey.KEIYAKU_KBN_MAE, pSyoriKbn)
                ...
                '    '変更前　契約区分
                '    wkAR.pKeiyakuKbnMae = pCurrentKey.KEIYAKU_KBN_MAE
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSql As String) As Boolean
        ...
        Try
            ...
            wSql &= "SELECT" & vbCrLf
            wSql &= "  TRIM(TO_CHAR(WK1.KEIYAKUSYA_CD,'00000')) KEIYAKUSYA_CD," & vbCrLf
            '--契約者マスタ
            ...
            wSql &= "  '(' || TRIM(TO_CHAR(WK1.KEIYAKUSYA_CD,'00000')) || ')' KEIYAKUSYA_CD_KAKKO," & vbCrLf
            '--協会マスタ
            ...
            wSql &= "        AND KEIYAKUSYA_CD = " & pCurrentKey.KEIYAKUSYA_CD & vbCrLf
            ...
            End If
            wSql &= "      WHERE KI = " & pCurrentKey.KI & vbCrLf
            wSql &= "        AND SAKI_SEIKYU_KAISU = " & pCurrentKey.SEIKYU_KAISU & vbCrLf
            wSql &= "        AND KEIYAKUSYA_CD = " & pCurrentKey.KEIYAKUSYA_CD & vbCrLf
            wSql &= "      GROUP BY" & vbCrLf
            ...
            wSql &= "   WHERE ATU.KEIYAKUSYA_CD = MTU.KEIYAKUSYA_CD" & vbCrLf
            wSql &= "     AND ATU.KEIYAKUSYA_CD = AME.KEIYAKUSYA_CD" & vbCrLf
            wSql &= "     AND AME.KEIYAKUSYA_CD = MME.KEIYAKUSYA_CD" & vbCrLf
            ...
            wSql &= "  AND WK1.KEIYAKUSYA_CD = KYK.KEIYAKUSYA_CD(+)" & vbCrLf
            '--単価マスタ
