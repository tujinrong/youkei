...
'*　　①　フォーム名　　　  frmGJ2050.vb
'*
'*　　②　機能概要　　　　　互助金生産者積立金等請求書兼返還通知書（返還）発行処理
'*
'*　　③　作成日　　　　　　2015/03/03  BY JBD
'*
'*　　④　更新日            
'*
'*******************************************************************************
...
#Region "frmGJ2050_Load Form_Loadイベント"

    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ2050_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ2050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSql As String) As Boolean
        ...
        Try
            ...
            wSql &= "SELECT"
            wSql &= "  WK1.KI,"
            wSql &= "  '内　'||WK1.TESURYO_SYOHIZEI_RITU || '％消費税' TESURYO_SYOHIZEI_RITU,"    '2023/08/16　追加 インボイス対応　JBD453
            wSql &= "  WK1.TESURYO_SYOHIZEI TESURYO_SYOHIZEI,"                                   '2023/08/16　追加 インボイス対応　JBD453
            wSql &= "  WK1.SAGAKU_TESURYO_SYOHIZEI SAGAKU_TESURYO_SYOHIZEI,"                     '2023/08/16　追加 インボイス対応　JBD453
            ...
            '  --協会マスタ
            wSql &= "  '(登録番号：' || KYO.TOUROKU_NO  || ')' TOUROKU_NO," '2023/08/16　追加 インボイス対応　JBD453
            ...
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"        '2023/08/16　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"                  '2023/08/16　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI,"    '2023/08/16　追加 インボイス対応　JBD453
            ...
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"          '2023/08/16 インボイス対応 JBD453　追加
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"                    '2023/08/16 インボイス対応 JBD453　追加
            wSql &= "        MAX(TUM.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI,"      '2023/08/16 インボイス対応 JBD453　追加
            wSql &= "        MEI.TORI_KBN,"
            ...
            '  --前期積立金返還
            'wSql &= "  AND WK1.ZEN_KI = ZEN.KI(+)"
            wSql &= "  AND WK1.KI = ZEN.KI(+)"
            wSql &= "  AND WK1.KEIYAKUSYA_CD = ZEN.KEIYAKUSYA_CD(+) "
            '--出力順
            wSql &= "ORDER BY"
            'wSql &= "  KYK.JIMUITAKU_CD, WK1.KEIYAKUSYA_CD"    '2016/01/06　修正
            wSql &= "  WK1.KEIYAKUSYA_CD"                       '2016/01/06　修正
