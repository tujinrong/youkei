...
Namespace WebService.Common.Utils
    Public Module CmFileUtil
        Private ReadOnly DEFAULT_MIME As String = "application/octet-stream"
...
''' Input:
''' 
'''         private static readonly System.Collections.Generic.Dictionary<string, string> MappingDic = new(256, System.StringComparer.CurrentCultureIgnoreCase);
''' 
''' 
                ''' Cannot convert FieldDeclarationSyntax, System.InvalidCastException: 型 'Microsoft.CodeAnalysis.VisualBasic.Syntax.EmptyStatementSyntax' のオブジェクトを型 'Microsoft.CodeAnalysis.VisualBasic.Syntax.ExpressionSyntax' にキャストできません。
'''    場所 ICSharpCode.CodeConverter.VB.CommonConversions.RemodelVariableDeclaration(VariableDeclarationSyntax declaration)
...
''' Input:
''' 
'''         private static readonly System.Collections.Generic.HashSet<JBD.GJS.Common.EnumFileTypeKbn> ImgKbns = new();
''' 
''' 
        ...
        ''' 画像フラグ取得
        ''' </summary>
        Public Function GetImgFlg(ByVal fileType As Integer) As Boolean
            Return True ' CmFileUtil.ImgKbns.Contains(fileType)
        End Function
        ...
        Private Sub AddMapping(ByVal extension As String, ByVal mimeType As String)
            'CmFileUtil.MappingDic.TryAdd(extension, mimeType)
        End Sub

        Private Sub InitImgKbns()
            'CmFileUtil.ImgKbns.Add(EnumFileTypeKbn.png)
            'CmFileUtil.ImgKbns.Add(EnumFileTypeKbn.jpg)
            'CmFileUtil.ImgKbns.Add(EnumFileTypeKbn.jpeg)
            'CmFileUtil.ImgKbns.Add(EnumFileTypeKbn.tif)
        End Sub
        Private Sub Init()
            Call InitImgKbns()
