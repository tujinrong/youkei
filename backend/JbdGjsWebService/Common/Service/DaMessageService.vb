...
Namespace JBD.GJS.Common
    Public Class DaMessageService
        ...
        Public Shared Function GetMsg(ByVal id As EnumMessage, ParamArray param As Object()) As String
            Dim m = GetMsgModel(id).Msg & "(" & GetMsgModel(id).MsgNo & ")"
            Return String.Format(m, param)
        End Function
        Public Shared Function GetMsg(ByVal msgNo As String, ParamArray param As Object()) As String
        ...
        Public Shared Function GetMsgModel(ByVal id As EnumMessage) As MessageModel
            Return Nothing
            'Call ReadFile()
            ...
            '''' 
            '''' Input:
            'Dim key = System.Enum.GetName(TypeOf (JBD.GJS.Common.EnumMessage), id)
            '''' 
            '''' 
        ...
        Private Shared Sub ReadFile(ByVal file As String, ByVal dic As Dictionary(Of String, MessageModel))
            Dim loc = AppDomain.CurrentDomain.BaseDirectory & "bin"
            Dim fPath As String
            If System.IO.Directory.Exists(loc) Then
                fPath = System.IO.Path.Combine(loc, "Resource", file)
            Else
            ...
            Using sr = New System.IO.StreamReader(fPath, enc)
                While sr.Peek() >= 0
        ...
        Private Shared Function GetWords(ByVal s As String) As String()
    ...
    Public Class MessageViewModel
        ...
        Public Sub New(ByVal desc As String, ByVal MsgNo As String, ByVal Msg As String)
            Key = desc
            MessageNo = MsgNo
            Message = Msg
        End Sub
