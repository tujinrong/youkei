...
namespace BCC.Affect.Service.AWBH00501G
{
    ...
    public class Service : CmServiceBase
    {
        ...
        private CommonResponse ProcDelete(DaDbContext db, PasentairuListVM req)
        {
            ...
            //-------------------------------------------------------------
            //乳幼児パーセンタイル値マスタ
            db.tm_bhnypasentairu.DELETE.WHERE.ByKey(req.buicd, req.sex, req.month, req.date).Execute();
        ...
        private CommonResponse ProcInsert(DaDbContext db, PasentairuListVM req)
        {
            ...
            //追加処理
            //-------------------------------------------------------------
            var newdtl = new List<tm_bhnypasentairuDto>();
            ...
            //母子保健パーセンタイル値マスタ
            db.tm_bhnypasentairu.INSERT.Execute(newdtl); 
        ...
        private CommonResponse ProcUpdate(DaDbContext db, PasentairuListVM req)
        {
            ...
            //乳幼児パーセンタイル値マスタ
            var newdtl = new List<tm_bhnypasentairuDto>();    //新規追加の場合利用
            var upddtl = new List<tm_bhnypasentairuDto>();    //更新の場合利用
            ...
            {
                //更新の場合
                db.tm_bhnypasentairu.UPDATE.Execute(upddtl);
            }
        ...
        {
            //乳幼児パーセンタイル値マスタ
            var dto = db.tm_bhnypasentairu.GetDtoByKey(req.buicd, req.sex, req.month, req.date, pasentairucd!);
            if (dto != null)
            {
                //存在する場合
