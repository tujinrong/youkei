...
namespace BCC.Affect.Service.AWKK00801G
{
    public class Wraper : CmWraperBase
    {
        ...
        public static SidoCommonHeaderVM GetHeaderVM(DaDbContext db, SidoCommonRequest req)
        {
            ...
            vm.gyomukbnnm = DaNameService.GetName(db, EnumNmKbn.拡張_予約_保健指導業務区分, req.gyomukbn);              //業務区分名称
        ...
        {
            //業務区分一覧
            var list = DaNameService.GetSelectorList(db, EnumNmKbn.拡張_予約_保健指導業務区分, Enum名称区分.名称);
        ...
        private static YoyakuSidoJigyoRowVM GetVM(DaDbContext db, DataRow row)
        {
            ...
            //業務区分(コード：名称)
            vm.gyomukbncdnm = DaNameService.GetCdNm(db, EnumNmKbn.拡張_予約_保健指導業務区分, Enum名称区分.名称, gyomukbn);
        ...
        private static KensinListRowVM GetVM(DaDbContext db, tm_afhanyoDto dto, tm_shkensinjigyoDto dto2)
        {
            ...
            vm.jigyokbn = dto2.kihonkakutyokbn;                                                                         //基本／拡張事業区分
            vm.jigyokbnnm = DaNameService.GetName(db, EnumNmKbn.事業区分, EnumToStr(dto2.kihonkakutyokbn));             //基本／拡張事業区分名
        ...
        private static ItemListRowBaseVM GetVM(DaDbContext db, ItemListRowBaseVM vm, string itemcd, string itemnm, Enumフリー項目区分区分 itemkbn, Enum入力タイプ inputtype)
        {
            vm.itemcd = itemcd;                                                                                         //項目コード
            vm.itemnm = itemnm;                                                                                         //項目名
            vm.kakutyokbn = itemkbn == Enumフリー項目区分区分.PKG標準項目拡張領域不使用 ? "―" : "○";                  //拡張領域使用
            vm.pkgdokujikbn = vm.kakutyokbn == "○" ? Enum事業区分.市区町村拡張事業 : Enum事業区分.基本事業;            //PKG標準・独自区分
