...
namespace BCC.Affect.Service.AWKK00701G
{
    ...
    public class Service : CmServiceBase
    {
        ...
        public GetTargetItemValueResponse GetTargetItemValue(GetTargetItemValueRequest req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                ...
                foreach (var dto in dtl)
                {
                    ...
                    if (!string.IsNullOrEmpty(dto.itemtokuteikbn) && dto.itemtokuteikbn.Equals(EnumToStr(Enum項目特定区分.生年月日)))
                    {
                        var dt1 = db.tt_afatena.SELECT.WHERE.ByKey(req.val).GetDto();
                        string bymdName = CmLogicUtil.GetYMDStr(dt1.bymd_fusyoflg, dt1.bymd, dt1.bymd_fusyohyoki);
                        value = bymdName;
                    }
                    //取得先項目ID,取得先項目の値
                ...
                if (dto != null && !string.IsNullOrEmpty(dto.itemtokuteikbn))
                {
                    switch (ParseEnum<Enum項目特定区分>(dto.itemtokuteikbn))
                    {
                        ...
                        case Enum項目特定区分.検診実施機関番号:
                            ...
                            break;
                        case Enum項目特定区分.宛名番号:
                        case Enum項目特定区分.把握経路:
                        case Enum項目特定区分.把握事業:
                        case Enum項目特定区分.把握日:
                            List<object[]>? followData = GetFollowByInputData(db, pageItemDtl, req.detailRowVM.PageItemBodyList);
                            if (followData != null)
                            {
                                res.targetItemValueList.AddRange(followData);
                            }
                            break;
                        default:
                            break;
        ...
        private ImResult ImportFromEditData(DaDbContext db, SaveRequest req, tm_kktorinyuryokuDto kkimpDto, List<tm_kktorinyuryoku_itemDto> kkimppageitemDtl, DataTable temptableData)
        {
            ...
            //TODO高_20240703
            //if (kkimpDto.torokukbn == EnumToStr(Enum登録区分.ストアドで登録))
            //{
            //    imDef.ProcName = kkimpDto.proctoroku;
            //}
        ...
        public static void SaveKkimpDetailData(DaDbContext db, int impexeid, List<ImEditDataRow> editDataList, string processKey)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //一括登録
                DaDbUtil.BulkInsert(db.Session, dt, tt_kktorinyuryoku_misyoriitemDto.TABLE_NAME, false);
            }
        }
        ...
        private static ImEnumImportType GetRegupdKbn(string regupdkbn)
        {
            switch (ParseEnum<Enum登録区分>(regupdkbn))
            {
                ...
                //TODO高_20240703
                //ストアドで登録
                //case Enum登録区分.ストアドで登録:
                //    return ImEnumImportType.StoredProcedure;
                default:
                    throw new NotImplementedException();
            }
        }
        ...
        /// </summary>
        /// <param name="db"></param>
        /// <param name="pageItemDtl"></param>
        /// <param name="pageItemBodyList"></param>
        /// <returns></returns>
        private List<object[]>? GetFollowByInputData(DaDbContext db, List<tm_kktorinyuryoku_itemDto> pageItemDtl, List<PageItemBodyModel> pageItemBodyList)
        {
            //TODO高_20240703
            ...
            var requiredValues = requiredItems
                .Select(kbn => FindValueByKbn(pageItemDtl, pageItemBodyList, kbn))
                .ToArray();

            //NULLの場合、 リターン
            if (requiredValues.Any(v => v == null))
            {
                return null;
            }
            //フォロー内容情報テーブルを取得
            var dtoList = db.tt_kkfollownaiyo.SELECT.WHERE.ByItemList(
                new[]
                {
                    nameof(tt_kkfollownaiyoDto.atenano),
                    nameof(tt_kkfollownaiyoDto.haakukeiro),
                    //nameof(tt_kkfollownaiyoDto.hakuymd),
                    nameof(tt_kkfollownaiyoDto.haakujigyocd)
                },
                new List<object[]> { requiredValues.Select(v => v!).ToArray() }).GetDtoList();
            ...
            //最初の値を取る
            var dto = dtoList[0];
            var optionalItems = new (Enum項目特定区分 kbn, object value)[]
            {
                (Enum項目特定区分.フォロー内容枝番, dto.follownaiyoedano),
                (Enum項目特定区分.フォロー内容, dto.follownaiyo ?? ""),
                (Enum項目特定区分.フォロー状況, dto.followstatus)
            };
            //フォロー結果を取得
            var result = optionalItems
                .Select(tuple => new { Item = FindItemByKbn(pageItemDtl, tuple.kbn), tuple.value })
                .Where(x => x.Item != null)
                .Select(x => new object[] { x.Item!.gamenitemseq, x.value })
                .ToList();
        ...
        /// <returns></returns>
        private static string? FindValueByKbn(List<tm_kktorinyuryoku_itemDto> pageItemDtl, List<PageItemBodyModel> pageItemBodyList, Enum項目特定区分 kbn)
        {
            var item = pageItemDtl.Find(e => e.itemtokuteikbn == EnumToStr(kbn));
            return item != null ? pageItemBodyList.Find(e => e.pageitemseq == item.gamenitemseq)?.val : null;
        }
        ...
        /// <param name="kbn"></param>
        /// <returns></returns>
        private static tm_kktorinyuryoku_itemDto? FindItemByKbn(List<tm_kktorinyuryoku_itemDto> pageItemDtl, Enum項目特定区分 kbn)
        {
            return pageItemDtl.Find(e => e.itemtokuteikbn == EnumToStr(kbn));
        }
