...
namespace BCC.Affect.Service.AWKK00111G
{
    public class Wraper : CmWraperBase
    {
        ...
        /// <summary>
        /// 世帯情報(詳細画面)
        public static SetaiInfoVM GetVM(DaDbContext db, tt_afjutogaiDto dto)
        {
            var vm = new SetaiInfoVM();

            vm.setaino = CStr(dto.setaino);                                                                 //世帯番号
            vm.simei_yusen = CStr(dto.simei_yusen);                                                         //世帯主氏名(氏名_優先)
            vm.adrs_yubin = CStr(dto.adrs_yubin);                                                           //郵便番号
            vm.tosi_gyoseikucd = MCdNm(db, CStr(dto.tosi_gyoseikucd), EnumNmKbn.指定都市_行政区等コード);   //指定都市_行政区等コード（コード：名称）
            vm.adrs_sikucd = MCdNmExp(dto.adrs_sikucd, dto.adrs_todofuken, dto.adrs_sikunm);                //市区町村
            vm.adrs_todofuken = CStr(dto.adrs_todofuken);                                                   //住所_都道府県
            vm.adrs_sikunm = CStr(dto.adrs_sikunm);                                                         //住所_市区郡町村名
            vm.adrs_tyoazacd = $"{dto.adrs_tyoazacd}{C_SPACE}{C_COLON}{C_SPACE}{dto.adrs_tyoaza}";          //住所_町字コード
            vm.adrs_tyoaza = CStr(dto.adrs_tyoaza);                                                         //町字名
