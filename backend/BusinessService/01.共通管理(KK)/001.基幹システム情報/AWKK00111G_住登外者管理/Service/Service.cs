...
namespace BCC.Affect.Service.AWKK00111G
{
    ...
    public class Service : CmServiceBase
    {
        ...
        [DisplayName("世帯検索処理(詳細画面)")]
        public SearchSetaiResponse SearchSetai(SearchSetaiRequest req)
        {
            return Nolock(req, (DaDbContext db) =>
            {
                //正常返し
                return SearchSetai(db, req);
            });
        }
        ...
        /// 世帯検索処理(詳細画面)
        /// </summary>
        private SearchSetaiResponse SearchSetai(DaDbContext db, SearchSetaiRequest req)
        {
            var res = new SearchSetaiResponse();

            //-------------------------------------------------------------
            //１.データ取得
            //-------------------------------------------------------------
            var dtl = db.tt_afjutogai.SELECT.WHERE.ByFilter($"{nameof(tt_afjutogaiDto.setaino)}=@nmmaincd and " +
                                                            $"{nameof(tt_afjutogaiDto.stopflg)}=false"
                                                            , req.setaino).GetDtoList().OrderByDescending(e => e.rirekino);
            if (dtl == null) { return res; }

            var atenadto = new tt_afatenaDto();
            var jutogaidto = new tt_afjutogaiDto();

            foreach (var dto in dtl)
            {
                atenadto = db.tt_afatena.SELECT.WHERE.ByKey(dto.atenano).GetDto();
                if (atenadto != null && (CStr(atenadto.zokucd1).Equals(続柄._02) || CStr(atenadto.zokucd2).Equals(続柄._02) || CStr(atenadto.zokucd3).Equals(続柄._02) || CStr(atenadto.zokucd4).Equals(続柄._02)))
                {
                    jutogaidto = dto;
                    break;
                }
            ...
            res.setaiInfo = Wraper.GetVM(db, jutogaidto);
        ...
        private CommonResponse Save(DaDbContext db, SaveRequest req, string jigyocd, string kinoid)
        {
            ...
            if (kbn == EnumActionType.Update)
            {
                ...
                oldpersonalno = CStr(db.tt_afatena.GetDtoByKey(req.maininfo.atenano).personalno);
            ...
            //個人番号の正当性チェック
            if (!CheckPersonalno(req.maininfo.personalno!))
            {
                var msg = DaMessageService.GetMsg(EnumMessage.ITEM_ILLEGAL_ERROR, "個人番号");
                res.SetServiceError(msg);
                return res;   //異常返し
            }
        ...
        private SearchAutoSaibanResponse AutoSaisin(DaDbContext db, AutoSaibanRequest req)
        {
            ...
            if (req.seqflg.Equals("1"))
            {
                ...
                //② 処理①で取得した番号を元にチェックデジットを求める
                seq = GetSeqNo(db, req.seqflg);
            }
        ...
        /// シーケンス値を取得
        /// </summary>
        public static string GetSeqNo(DaDbContext db, string seqflg)
        {
            var seqno = string.Empty;
            ...
            {
                //世帯番号の場合
                sql = "SELECT nextval('sq_setai_no')";
            }
            else if (seqflg.Equals("2"))
            {
                //宛名番号の場合
                sql = "SELECT nextval('sq_afatena_no')";
            }

            DataTable dt = (result.Data = AiDbUtil.GetDataTable(db.Session, sql));
            seqno = CStr(dt.Rows[0][0]);
        ...
        /// シーケンス値を取得
        /// </summary>
        public static bool CheckPersonalno(string personalno)
        {
            if (string.IsNullOrEmpty(personalno)) { return true; }

            if (personalno.Length != 12) { return false; }

            var checkbit = CalculateCheckDigit(personalno.Substring(0, 11));
            var lastbit = personalno.Substring(11, 1);
            if (CStr(checkbit) == lastbit) { return true; }
            return false;
        }
