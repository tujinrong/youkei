    ...
    public class Service : CmServiceBase
    {
        ...
        public ReportPreviewResponse Preview(PreviewRequest req)
        {
            return Nolock(req, db =>
            {
                ...
                //出力情報
                var outputInfoDic = CommonUtil.GetOutPutInfo(req.outPutInfo, req.outputfilenm, null, string.Empty);
        ...
        public CmDownloadResponseBase Download(DownloadRequest req)
        {
            return Nolock(req, db =>
            {
                ...
                //zip対応by屠
                if (result.FileFullPath.EndsWith("zip"))
                {
                    fileSuffix = ".zip";
                }
        ...
        public DaResponseBase AddBatchJob(BatchRequest req)
        {
            return Transction(req, db =>
            {
                ...
                //var result = DaEucService.MakeSheet(db, req.workseq.ToString(), req.rptid, req.yosikiid, req.outputtype, searchDic, detailSearchDic, outputInfoDic, orderByList, req.memo ?? string.Empty,
                //    req.sessionid.ToString(), req.userid, req.regsisyo, false, req.hakusiflg, req.jyokensheetflg, req.rirekiupdflg,true);
                ...
                //    else
                //    {
                //        var msg = DaMessageService.GetMsg(EnumMessage.E004007, "バッチ処理");
                //        res.SetServiceError(msg);
                //        return res;
                //    }
                //}
                ...
                var eurirekiDto = Converter.GeteurirekiDto(req, jpExecutionTime);
                db.tt_eurireki.INSERT.Execute(eurirekiDto);

                var parameterModel = Wraper.GetBatchParameter(req, nowFlg, eurirekiDto.rirekiseq, nextTaskId);
