...
namespace BCC.Affect.Service.AWYS00101G
{
    ...
    public class Wraper : CmWraperBase
    {
        ...
        public static SessyuOneVm GetVM(DaDbContext db, DataRow row)
        {
            ...
            vm.sessyu = GetCdNm(db, EnumHanyoKbn.接種種類詳細コード, Enum名称区分.名称, sessyucd);              //接種種類（コード : 名称）
            ...
            vm.sessyukbn = GetCdNm(db, EnumNmKbn.接種区分, Enum名称区分.名称, vm.sessyukbncd);                  //接種区分（コード : 名称）
            vm.lotno = CStr(row.Wrap(nameof(vm.lotno)));                                                        //ロット番号
            vm.vaccinenmcd = CStr(row.Wrap(nameof(vm.vaccinenmcd)));                                            //ワクチン名（コード)
            vm.vaccinenm = GetCdNm(db, EnumHanyoKbn.ワクチン, Enum名称区分.名称, vm.vaccinenmcd);               //ワクチン名（コード : 名称）
            vm.vaccinemakercd = CStr(row.Wrap(nameof(vm.vaccinemakercd)));                                      //チンメーカー（コード)
            vm.vaccinemaker = GetCdNm(db, EnumHanyoKbn.ワクチンメーカー, Enum名称区分.名称, vm.vaccinemakercd); //チンメーカー（コード : 名称）
            vm.sessyuryo = CDbl(row.Wrap(nameof(vm.sessyuryo)));                                                //接種量
            vm.jissikbncd = CStr(row.Wrap(nameof(vm.jissikbn)));                                                //実施区分（コード)
            vm.jissikbn = GetCdNm(db, EnumNmKbn.予防接種の実施方式, Enum名称区分.名称, vm.jissikbncd);          //実施区分（コード : 名称）
            vm.hoteikbncd = CStr(row.Wrap(nameof(vm.hoteikbn)));                                                //法定区分（コード)
            vm.hoteikbn = GetCdNm(db, EnumNmKbn.予防接種区分, Enum名称区分.名称, vm.hoteikbncd);                //法定区分（コード : 名称）
            vm.tokubetunojijyocd = CStr(row.Wrap(nameof(vm.tokubetunojijyo)));                                  //特別の事情（コード)
            vm.tokubetunojijyo = GetCdNm(db, EnumNmKbn.特別の事情, Enum名称区分.名称, vm.tokubetunojijyocd);    //特別の事情（コード : 名称）
        ...
        public static SessyuRikanVm GetVM(DaDbContext db, tt_ysrikanDto dto, List<string> sisyoList)
        {
            var vm = new SessyuRikanVm()
            {
                ...
                //表示項目
                rikan = GetCdNm(db, EnumHanyoKbn.罹患項目, Enum名称区分.名称, CStr(dto.rikancd)),                   //罹患種類（コード : 名称）
        ...
        public static FixItemSessyuVM GetSessyuFixVM(DaDbContext db, tt_yssessyuDto? dto, FixItemSessyuVM vm, EnumActionType? kbn)
        {
            ...
            {
                //非表示項目
                vm.sessyucd = CStr(dto.sessyucd);                                                            //接種種類コード（キー項目）
                vm.kaisu = CStr(dto.kaisu);                                                                  //回数（キー項目）
                vm.edano = CInt(dto.edano);                                                                  //枝番（キー項目）
                vm.sessyukbncd = CStr(dto.sessyukbn);                                                        //接種区分コード
                vm.vaccinenmcd = CStr(dto.vaccinenmcd);                                                      //ワクチン名コード
                vm.vaccinemakercd = CStr(dto.vaccinemakercd);                                                //ワクチンメーカーコード
                vm.jissikbncd = CStr(dto.jissikbn);                                                          //実施区分（コード)
                vm.hoteikbncd = CStr(dto.hoteikbn);                                                          //法定区分（コード)
                vm.jissikikancd = CStr(dto.jissikikancd);                                                    //実施機関（コード)
                vm.kaijocd = CStr(dto.kaijocd);                                                              //会場（コード)
                vm.isicd = CStr(dto.isicd);                                                                  //医師（コード)
                vm.tokubetunojijyocd = CStr(dto.tokubetunojijyo);                                            //特別の事情（コード)

                //表示項目
                var sessyucd = $"{vm.sessyucd}{vm.kaisu}";                                                   //接種種類（コード + 回数）
                vm.sessyu = GetDispItem(vm, 固定項目区分._接種_接種種類, sessyucd);                          //接種種類
                vm.jissiymd = CStr(dto.jissiymd);                                                            //実施日
                vm.sessyukbn = GetDispItem(vm, 固定項目区分._接種_接種区分, vm.sessyukbncd);                 //接種区分
                vm.lotno = CStr(dto.lotno);                                                                  //ロット番号
                vm.vaccinenm = GetDispItem(vm, 固定項目区分._接種_ワクチン名, vm.vaccinenmcd);               //ワクチン名
                vm.vaccinemaker = GetDispItem(vm, 固定項目区分._接種_ワクチンメーカー, vm.vaccinemakercd);   //ワクチンメーカー
                vm.sessyuryo = CDbl(dto.sessyuryo);                                                          //接種量
                vm.sessyukenno = CStr(dto.sessyukenno);                                                      //接種券番号
                vm.jissikbn = GetDispItem(vm, 固定項目区分._接種_実施区分, vm.jissikbncd);                   //実施区分
                vm.hoteikbn = GetDispItem(vm, 固定項目区分._接種_法定区分, vm.hoteikbncd);                   //法定区分
                vm.jissikikan = GetDispItem(vm, 固定項目区分._接種_実施機関, vm.jissikikancd);               //実施機関
                vm.jissikikannm = CStr(dto.jissikikannm);                                                    //実施機関名
                vm.kaijo = GetDispItem(vm, 固定項目区分._接種_会場, vm.kaijocd);                             //会場
                vm.kaijonm = CStr(dto.kaijonm);                                                              //会場名
                vm.isi = GetDispItem(vm, 固定項目区分._接種_医師, vm.isicd);                                 //医師
                vm.isinm = CStr(dto.isinm);                                                                  //医師名
                vm.tokubetunojijyo = GetDispItem(vm, 固定項目区分._接種_特別の事情, vm.tokubetunojijyocd);   //特別の事情
            }
            return vm;
        }
        ...
        public static FixItemFusinVM GetFusinFixVM(DaDbContext db, tt_ysfusinkotaiDto? dto, FixItemFusinVM vm, EnumActionType? kbn)
        {
            ...
            {
                //非表示項目
                vm.kotaikensahohocd = CStr(dto.kotaikensahohocd);                                           //抗体検査方法コード
                vm.tanicd = CStr(dto.tanicd);                                                               //単位コード
                vm.sessyuhanteicd = CStr(dto.sessyuhanteicd);                                               //接種判定コード
                vm.jissikikancd = CStr(dto.jissikikancd);                                                   //実施機関（コード)
                vm.isicd = CStr(dto.isicd);                                                                 //医師（コード)
                vm.kotaikensanocd = CStr(dto.kotaikensanocd);                                               //抗体検査番号コード

                //表示項目
                vm.sessyukenno = CStr(dto.sessyukenno);                                                     //接種券番号
                vm.kotaikensahoho = GetDispItem(vm, 固定項目区分._風疹_抗体検査方法, vm.kotaikensahohocd);  //抗体検査方法
                vm.kotaika = CStr(dto.kotaika);                                                             //抗体価
                vm.tani = GetDispItem(vm, 固定項目区分._風疹_単位, vm.tanicd);                              //単位（抗体価）
                vm.sessyuhantei = GetDispItem(vm, 固定項目区分._風疹_接種判定, vm.sessyuhanteicd);          //接種判定
                vm.jissiymd = CStr(dto.jissiymd);                                                           //実施日
                vm.jissikikan = GetDispItem(vm, 固定項目区分._風疹_実施機関, vm.jissikikancd);              //実施機関
                vm.jissikikannm = CStr(dto.jissikikannm);                                                   //実施機関名
                vm.isi = GetDispItem(vm, 固定項目区分._風疹_医師, vm.isicd);                                //医師
                vm.isinm = CStr(dto.isinm);                                                                 //医師名
                vm.kotaikensano = GetDispItem(vm, 固定項目区分._風疹_抗体検査番号, vm.kotaikensanocd);      //抗体検査番号
            }
            return vm;
        }
