...
*******************************************************************/
/*==============================head==============================*/
--DROP FUNCTION public.sp_awys00101g_01_head( character varying,  character varying,  integer,  character varying,  boolean,  integer)
CREATE OR REPLACE FUNCTION public.sp_awys00101g_01_head(atenano_in character varying, hanyomaincd_in character varying, hanyosubcd_in integer, rirekihyoji_in boolean, kbn_in integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
declare 
          ...
          FROM
            ...
            LEFT JOIN (
                SELECT 
                      ...
                      ,X.tokubetunojijyo
                  FROM tt_yssessyu X
                 WHERE (atenano_in is null OR X.atenano = atenano_in)
                ) A
...
BEGIN
    IF rirekihyoji_in = true THEN
        ...
        SELECT
               A.atenano
              ,SUBSTRING(H.hanyocd,1,5) AS sessyucd
              ,SUBSTRING(H.hanyocd,6,2) AS kaisu
              ,A.edano
              ,A.jissiymd
              ,A.sessyukbn
              ,A.lotno
         ...
         ORDER BY
             case when orderby_in = 0 then A.edano  end                 --枝番        昇順（デフォルト）
