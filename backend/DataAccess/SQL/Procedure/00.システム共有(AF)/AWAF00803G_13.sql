...
変更履歴　:
*******************************************************************/
create or replace function sp_awaf00803g_13(logkbn_in integer, status_in character varying, 
 ...
 (セッションシーケンス bigint,
  ...
  登録日時 text,
  項目値_項目値変更ログシーケンス bigint,
  項目値_セッションシーケンス bigint,
  項目値_テーブル名 character varying,
  項目値_変更区分 text,
  項目値_キー character varying,
  項目値_項目名 character varying,
  項目値_変更前内容 character varying,
  項目値_更新後内容 character varying,
  "項目値_登録ユーザーID" character varying,
  項目値_登録日時 text)
 language plpgsql
as $function$
declare
...
begin
    ...
    select 
        ...
        ,t.columnlogseq as 項目値_項目値変更ログシーケンス
        ,t.seq2 as 項目値_セッションシーケンス
        ,t.tablenm as 項目値_テーブル名
        ,t.henkokbn || ' : ' || t.henkokbnnm as 項目値_変更区分
        ,t.keys as 項目値_キー
        ,t.itemnm as 項目値_項目名
        ,t.valuebefore as 項目値_変更前内容
        ,t.valueafter as 項目値_更新後内容
	    ,t.reguserid2 as "項目値_登録ユーザーID"
        ,to_char(t.regdttm2, 'YYYY/MM/DD HH24:MI:SS') as 項目値_登録日時
    from (
        select
	    	...
	    	,t9.columnlogseq				--項目値変更ログシーケンス
            ,t9.sessionseq as seq2          --セッションシーケンス
            ,t9.tablenm                     --テーブル名
            ,t9.henkokbn                    --変更区分
            ,m2.nm henkokbnnm               --変更区分名称
            ,t9.keys                        --キー
            ,t9.itemnm                      --項目名
            ,t9.valuebefore                 --変更前内容
            ,t9.valueafter                  --更新後内容
        ...
        inner join tt_aflogcolumn t9                --テーブル項目値変更ログテーブル
            on t0.sessionseq = t9.sessionseq
        left join
            (select nmcd, nm 
                from tm_afmeisyo
                where nmmaincd = '1000'
                and nmsubcd = 8) m2                 --名称マスタ
                on t9.henkokbn = m2.nmcd
        where 
