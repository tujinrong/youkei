...
CREATE OR REPLACE FUNCTION fn_eueditbarcode(yubin_in character varying, jusyo_in character varying)
...
BEGIN
	...
	pdata := fn_euCngSanyoSuji(pdata);

	-- 正規表現を使って、算用数字、ハイフン、1文字のアルファベット(連続していないアルファベット)を抜き出す
	pdata := yubin_in || fn_euCngRegExp(pdata);
