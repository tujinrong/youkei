    ...
    public class KensinyoyakukibosyaRelation : IFrRelation
    {
        ...
        public List<FrTableRelation> GetRelations(List<FrSelectItem> itemList, FrCondition condition, params string[]? paras)
        {
                ...
                || itemList.Exists(e => e.TableName == tt_afatenaDto.TABLE_NAME))
            {
                ...
                if (condition.ConditionModel.ConditionList.Count() > 0 && 
                    condition.ConditionModel.ConditionList.Last().ConditionOprator != FrEnumConditionOprator.NONE) {
                    condition.ConditionModel.ConditionList.RemoveAt(condition.ConditionModel.ConditionList.Count() - 1);
                }
