...
namespace BCC.Affect.DataAccess
{
    ...
    public static partial class DaEucService
    {
        ...
        /// 帳票出力履歴テーブルに情報登録
        /// </summary>
        public static void OutPutHistoryInfo(DaDbContext db, string rptID, string yosikiID, EnumReportType reportType, Dictionary<string, object> searchDic,
            string memo, string sessionseq, string reguserid, AIplus.AiReport.Common.ArResult result, DateTime syoridttmf, DateTime syoridttmt
            , Dictionary<string, object> outputInfoDic, Enum履歴出力状態区分 jyotaikbn)
        {
            ...
            //履歴シーケンス
            var rirekiseq = outputInfoDic[CONDITION_RIREKI_SEQ];

            // 帳票出力履歴テーブルに登録
            if (string.IsNullOrEmpty(CStr(rirekiseq)))
            {
                // EUC帳票出力履歴テーブルに出力
                ...
                rirekidto.memo = memo;
                if (outputInfoDic.Count > 0 && outputInfoDic.TryGetValue(CONDITION_FILE_NM, out object fileNmValue))
                {
                    string fileNm = CStr(fileNmValue);
                    if (!string.IsNullOrEmpty(fileNm))
                    {
                        rirekidto.filenm = fileNm;
                    }
                }
                rirekidto.jyotaikbn = jyotaikbn;
                ...
                rirekiseq = rirekidto.rirekiseq;
            }
            else
            {
                // EUC帳票出力履歴テーブルに出力
                var rirekidto = db.tt_eurireki.GetDtoByKey(CLng(rirekiseq));
                rirekidto.jyotaikbn = jyotaikbn;
                rirekidto.num = result.DataCount;
                rirekidto.syoridttmf = syoridttmf;
                rirekidto.syoridttmt = syoridttmt;
                rirekidto.milisec = (int)(syoridttmt - syoridttmf).TotalMilliseconds;
                rirekidto.filedata = result.FileData;
                db.tt_eurireki.UPDATE.Execute(rirekidto);
            }
            // EUC帳票出力履歴条件に出力
            GetsearchDic(db, searchDic, CLng(rirekiseq));
        }

        public static void GetsearchDic(DaDbContext db, Dictionary<string, object> searchDic, long rirekiseq)
        {
            // EUC帳票出力履歴条件に出力
            var index = 0;
            ...
            ref ArSheetDefBase subSheetBase, string rptID, string yosikiID, EnumReportType reportType, string csvPattern, Dictionary<string, object> searchDic, tm_euyosikiDto rptSheet, string reguserid, DaSystemParameterManager sysParaManger)
        {
            ...
            foreach (var item in items)
            {
                ...
                if (string.IsNullOrEmpty(csvPattern) || ptnList!.Exists(e => e.reportitemid == item.yosikiitemid))
                {
                    ...
                    itemdef.SortTypeSeq = (item != null && item!.orderkbnseq != 0)  ? item.orderkbnseq : 0;
