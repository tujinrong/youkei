...
namespace BCC.Affect.DataAccess
{
    public static partial class DaEucService
    {
             ...
             string reguserid, string? regsisyo, bool hasUpdateSql = false, bool isBlank = false, bool hasConditionSheet = false, bool hasUpdHassoRireki = false, bool isBatch = false)
        {
            ...
            //PDF大量データの場合、並列処理を行う
            ArGlobal.IsParallel = true;                         //並列処理
            ArGlobal.MaxParallelThreads = 3;                    //スレッド数
            ArGlobal.SHEET_BREAK = 500;                         //改シートページ数
            ArGlobal.BOOK_BREAK = ArGlobal.SHEET_BREAK * 10;　  //改BOOKページ数
...
#if DEBUG
            const int RUNTIME_TIMEOUT = 3 * 60;
#else
            const int RUNTIME_TIMEOUT = 3 * 60;
#endif
            ...
            string reguserid, string? regsisyo, bool hasUpdateSql = false, bool isBlank = false, bool hasConditionSheet = false, bool hasUpdHassoRireki = false)
        {
            ...
            {
                { CONDITION_RIREKI_SEQ, "" },
                { CONDITION_FILE_NM, csvFmtDic[CONDITION_FILE_NM] }
            };
            // 帳票出力履歴テーブルに登録
            OutPutHistoryInfo(db, rptID, yosikiID, EnumReportType.CSV, searchDic, memo, sessionseq, reguserid, result, syoridttmf, syoridttmt, outputInfoDic, jyotaikbn);
            ...
            Dictionary<string, object> detailSearchDic, List<string> orderByList, string memo, string sessionseq, string reguserid, string? regsisyo, string csvPattern, bool isCsvFlg, bool isMakeWorkFLg, bool hasUpdHassoRireki, TransData? paramData = null)
        {
            ...
            foreach (var kvp in searchDic)
            {
                ...
                if (value != null)
                {
                    ...
                    {
                        var controlkbn = (Enumコントロール)kensakuDto.controlkbn;
                        //データ型の検証
                        if ((EnumDataType)kensakuDto.datatype != EnumDataType.文字列)
                        {
                            if (!IsValidCombination((EnumDataType)kensakuDto.datatype!, controlkbn, value))
                            {
                        ...
                        if ((int)EnumDataType.整数 == kensakuDto.datatype 
                            || (int)EnumDataType.小数 == kensakuDto.datatype)
                        {
                            if (controlkbn == Enumコントロール.文字入力 && !string.IsNullOrEmpty(CStr(kvp.Value)))
                            {
                                searchDic[key] = CInt(kvp.Value);
                            }
                            else if (controlkbn == Enumコントロール.文字範囲)
                            {
                                var values = (ArPair)kvp.Value;

                                if (!string.IsNullOrEmpty(CStr(values.First)))
                                {
                                    values.First = CInt(values.First);
                                }
                                if (!string.IsNullOrEmpty(CStr(values.Second)))
                                {
                                    values.Second = CInt(values.Second);
                                }

                                searchDic[key] = values;
                            }
                        }
                        else if ((int)EnumDataType.フラグ == kensakuDto.datatype)
                        {
                            if (controlkbn == Enumコントロール.文字入力 
                                || controlkbn == Enumコントロール.数値入力)
                            {
                                searchDic[key] = CBool(kvp.Value);
                            }
                        }
                    }
                }
        ...
        {
            int formatkbn = Convert.ToInt32(controlkbn);
            bool[,] supportArray = new bool[6, 13]
                            {
                            //数値, 文字, 日付, 時間, 数範囲, 文範囲,日範囲, 時範囲, 選択, 複選択,論理値,宛名入力,年度
                            { true, true, false, false,  true,  false, false, false, false, false, false, false, true}, // 整数
                            { true, true, false, false,  true, false, false, false, false, false, false, false, true}, // 小数
                            { true, true, true,  true,  true,  true,  true,  true,  true,  true,  true, true, true}, // 文字列
                            { false, true, true, true,  true,  true, true, true, false,  false, false, false, true}, // 日付
                            { false, true, false, false, false, true,  false, true, false,  false, false, false, false}, // 時間
                            { true, true, false, false,  false, false, false,  false, true,  false, true, false, false} // フラグ
                            };
        ...
        /// 種類の判断します
        /// </summary>
        public static bool IsValidCombination(EnumDataType datatype, Enumコントロール controlkbn, object syokiti)
        {
            switch (datatype)
            {
                case EnumDataType.整数:
                case EnumDataType.小数:
                    if (controlkbn == Enumコントロール.数値範囲
                        || controlkbn == Enumコントロール.数値入力
                        || controlkbn == Enumコントロール.年度)
                    {
                        return true;
                    }
                    else if (controlkbn == Enumコントロール.文字入力)
                    {
                        return IsParameterValid(datatype, syokiti.ToString());
                    }
                    else if (controlkbn == Enumコントロール.文字範囲)
                    {
                        return IsParameterArrValid(datatype, syokiti);
                    }
                    else
                    {
                        return false;
                ...
                case EnumDataType.フラグ:
                    if (controlkbn == Enumコントロール.論理値)
                    {
                        return true;
                    }
                    else if (controlkbn == Enumコントロール.文字入力 || controlkbn == Enumコントロール.数値入力)
                    {
                        return IsParameterValid(datatype, syokiti.ToString());
                    }
                    else
        ...
        /// 種類の判断する(範囲)
        /// </summary>
        public static bool IsParameterArrValid(EnumDataType? dataType, object parameterValue)
        {
            var values = (ArPair)parameterValue;

            if ((values.First == null || IsParameterValid(dataType, values.First.ToString()))
                && (values.Second == null || IsParameterValid(dataType, values.Second.ToString())))
            {
                return true;
            }
            return false;
