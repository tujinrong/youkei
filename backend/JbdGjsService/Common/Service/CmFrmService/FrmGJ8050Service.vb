...
#Region "f_Search_SQLMake ＳＱＬ作成処理"
        ...
        Public Shared Function f_Search_SQLMake2(wKbn As SearchSitenRequest) As String
            ...
            Else If wKbn.ORDER_BY = -3 Then
                wSql += " UTL_I18N.TRANSLITERATE(case when  STN.SITEN_KANA is null then '0' else TO_MULTI_BYTE(STN.SITEN_KANA) end, 'hwkatakana_fwkatakana')  DESC "     & vbCrLf
            Else If wKbn.ORDER_BY = 3 Then
                wSql += " UTL_I18N.TRANSLITERATE(case when  STN.SITEN_KANA is null then '0' else TO_MULTI_BYTE(STN.SITEN_KANA) end, 'hwkatakana_fwkatakana')  ASC "     & vbCrLf
            Else If wKbn.ORDER_BY = -4 Then
                wSql += " UTL_I18N.TRANSLITERATE(case when  STN.SITEN_NAME is null then '0' else TO_MULTI_BYTE(STN.SITEN_NAME) end, 'hwkatakana_fwkatakana')  DESC "     & vbCrLf
            Else If wKbn.ORDER_BY = 4 Then
                wSql += " UTL_I18N.TRANSLITERATE(case when  STN.SITEN_NAME is null then '0' else TO_MULTI_BYTE(STN.SITEN_NAME) end, 'hwkatakana_fwkatakana')  ASC "     & vbCrLf
            Else
