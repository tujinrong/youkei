    ...
    Public Module CmLogicUtil
        ...
        Public Function GetPersonalnoDB(personalno As String) As String
            If Not String.IsNullOrEmpty(personalno) Then
                Return CmEncryptUtil.RsaDecryptAndAesEncrypt(personalno)
            End If
            Return Nothing
        End Function
        ...
        ''' <returns></returns>
        Public Function GetKeikoku(db As DaDbContext, siensotikbn As String, alertviewflg As Boolean) As String
            Dim str = String.Empty
            If Not String.IsNullOrEmpty(siensotikbn) Then
                'str = If(alertviewflg, $"{WarningtextPrefix}{C_LEFT_BRACKET_FULL}{Gjs.DaNameService.GetName(db, Gjs.EnumNmKbn.支援措置区分, siensotikbn)}{C_RIGHT_BRACKET_FULL}", NoAuthWarningtext)
            End If
            Return str
        End Function
        ...
        ''' 住所取得
        ''' </summary>
        Public Function GetAdrs(jutokbn As Enum住登区分, adrs1 As String, adrs2 As String, adrsFlg As Boolean) As String
            '住登区分により表示パターンが異なる
            Select Case jutokbn
                Case Enum住登区分.住民
                    '住所
                    If adrsFlg Then
                        Return $"{adrs1}{adrs2}"
                    End If
                    Return DaConvertUtil.CStr(adrs2)
                Case Enum住登区分.住登外
                    '住所
                    Return $"{adrs1}{adrs2}"
                Case Else
        ...
        ''' 世帯主宛名番号取得
        ''' </summary>
        'Public Function GetSetainushi(dtl As List(Of tt_afatenaDto)) As String
        '    '世帯主コード
        ...
        ''' 警告参照フラグ取得
        ''' </summary>
        Public Function GetAlertviewflg(db As DaDbContext, token As String, userid As String, regsisyo As String) As Boolean
        ...
        ''' 更新可能支所一覧取得
        ''' </summary>
        Public Function GetSisyoList(db As DaDbContext, token As String, userid As String, regsisyo As String) As List(Of String)
            Dim dto = GetDto(db, token, userid, regsisyo)
        ...
        ''' 個人番号権限フラグ取得
        ''' </summary>
        Public Function GetPersonalflg(db As DaDbContext, token As String, userid As String, regsisyo As String, kinoid As String) As Boolean
            Dim dto = GetDto(db, token, userid, regsisyo)
        ...
        ''' 画面権限一覧取得
        ''' </summary>
        Public Function GetAuthList(db As DaDbContext, token As String, userid As String, regsisyo As String) As List(Of GamenModel)
            Dim dto = GetDto(db, token, userid, regsisyo)
        ...
        ''' 権限取得
        ''' </summary>
        Public Function GetAuthVM(db As DaDbContext, token As String, userid As String, regsisyo As String, kinoid As String) As CmAuthVM
            Dim vm = New CmAuthVM()
            Dim dto = GetDto(db, token, userid, regsisyo)
        ...
        ''' </summary>
        Public Function GetPnoeditflg(db As DaDbContext, token As String, userid As String, regsisyo As String) As Boolean
            Dim dto = GetDto(db, token, userid, regsisyo)
        ...
        ''' <returns></returns>
        'Public Function GetJigyocdList(db As DaDbContext, kbn As Enum事業コード区分, kinoid As String, no As String, hasStopFlg As Boolean) As List(Of String)
        '    '集約コードパターンEnum
        ...
        ''' <returns></returns>
        Public Function GetJigyocdList(db As DaDbContext, kbn As Enum事業コード区分, kinoid As String, kbn2 As Enum事業コードパターン, patternno As Integer, hasStopFlg As Boolean) As List(Of String)
            Dim list = GetJigyocdSelectorList(db, kbn, kinoid, Enum名称区分.名称, kbn2, patternno, hasStopFlg)
        ...
        ''' 事業コード一覧を取得
        ''' </summary>
        'Public Function GetJigyocdSelectorList(db As DaDbContext, kbn As Enum事業コード区分, kinoid As String, kbn2 As Enum名称区分, no As String, hasStopFlg As Boolean) As List(Of DaSelectorModel)
        '    '集約コードパターンEnum
        ...
        ''' 事業コード一覧を取得
        ''' </summary>
        Public Function GetJigyocdSelectorList(db As DaDbContext, kbn As Enum事業コード区分, kinoid As String, kbn2 As Enum名称区分, kbn3 As Enum事業コードパターン, patternno As Integer, hasStopFlg As Boolean) As List(Of DaSelectorModel)
            '汎用コードを取得
            Dim code = kinoid
        ...
        Public Function CIntJuminjotaiSort(juminjotai As String) As Integer
            Dim kbn = DaConvertUtil.CInt(juminjotai)
            If kbn = 1 Then
                Return 1
            Else
                Return 2
        ...
        ''' 一覧選択リスト(条件コード)
        ''' </summary>
        'Public Function GetFreeItemCdList(db As DaDbContext, nmkbn As Enum名称区分, inputtype As Enum入力タイプ, kbn As Enum条件コード区分, ParamArray keys As String()) As List(Of DaSelectorModel)
        '    Select Case inputtype
        '        Case Gjs.Enum入力タイプ.コード_名称マスタ
        '            Select Case kbn
        '                Case Enum条件コード区分.コード1
        '                    Return Gjs.DaNameService.GetSelectorList(db, Gjs.EnumNmKbn.名称マスタメインコード, nmkbn)
        '                Case Enum条件コード区分.コード2
        '                    If keys.Length = 0 Then
        '                        Return New List(Of DaSelectorModel)()
        '                    End If
        '                    Return Gjs.DaSelectorService.GetSelectorList(db, nmkbn, Gjs.Enumシステムマスタ区分.名称メインマスタ, keys)
        '                Case Enum条件コード区分.コード3
        '                    Return New List(Of DaSelectorModel)()
        '                Case Else
        '                    Throw New Exception("Enum条件コード区分 error")
        '            End Select
        '        Case Gjs.Enum入力タイプ.コード_汎用マスタ
        '            Select Case kbn
        '                Case Enum条件コード区分.コード1
        '                    Return Gjs.DaNameService.GetSelectorList(db, Gjs.EnumNmKbn.汎用マスタメインコード, nmkbn)
        '                Case Enum条件コード区分.コード2
        '                    If keys.Length = 0 Then
        '                        Return New List(Of DaSelectorModel)()
        '                    ElseIf keys.Length = 3 AndAlso Equals(Gjs.DaConvertUtil.CStr(keys(0)), 名称マスタ.システム.名称マスタメインコード._3019) Then
        '                        '検診フリー項目の場合
        '                        'var dtl = tm_afhanyo_main.SELECT.WHERE.ByKey(CStr(keys[0])).GetDtoList();
        '                        '成人健（検）診事業コード=>No.
        '                        Dim ct = Gjs.DaConvertUtil.CInt(keys(1))
        '                        'グループID
        '                        Dim kensinKbn = CType(Gjs.DaConvertUtil.CInt(keys(2)), EnumKensinKbn)
        '                        Dim subcdMin = 230001
        '                        Dim subcdMax = 270000
        '                        '標準
        '                        If ct <= AWSH001_CT Then
        '                            If kensinKbn = Gjs.EnumKensinKbn.一次 Then
        ...
        '                        End If
        '                    End If
        '                    Return Gjs.DaSelectorService.GetSelectorList(db, nmkbn, Gjs.Enumシステムマスタ区分.汎用メインマスタ, keys)
        '                Case Enum条件コード区分.コード3
        '                    Return New List(Of DaSelectorModel)()
        '                Case Else
        '                    Throw New Exception("Enum条件コード区分 error")
        '            End Select
        '        Case Gjs.Enum入力タイプ.コード_ユーザーマスタ
        '            Select Case kbn
        '                'Case Enum条件コード区分.コード1
        '                '    Return DaSelectorServiceHelpers.GetSelectorList(db, nmkbn, Gjs.Enumマスタ区分.所属グループマスタ)
        '                Case Enum条件コード区分.コード2, Enum条件コード区分.コード3
        '                    Return New List(Of DaSelectorModel)()
        '                Case Else
        '                    Throw New Exception("Enum条件コード区分 error")
        '            End Select
        '        Case Gjs.Enum入力タイプ.コード_地区情報マスタ
        '            Select Case kbn
        '                Case Enum条件コード区分.コード1
        '                    Return Gjs.DaNameService.GetSelectorList(db, Gjs.EnumNmKbn.地区区分, nmkbn)
        '                Case Enum条件コード区分.コード2, Enum条件コード区分.コード3
        '                    Return New List(Of DaSelectorModel)()
        '                Case Else
        '                    Throw New Exception("Enum条件コード区分 error")
        '            End Select
        '        Case Gjs.Enum入力タイプ.コード_所属グループマスタ, Gjs.Enum入力タイプ.コード_会場情報マスタ, Gjs.Enum入力タイプ.医療機関, Gjs.Enum入力タイプ.事業従事者
        '            Select Case kbn
        ...
        ''' 名称取得(項目)
        ''' </summary>
        Public Function GetFreeItemCdNm(db As DaDbContext, cd As String, kbn As Enumフリーマスタ分類, inputtype As Enum入力タイプ, codejoken1 As String, codejoken2 As String, codejoken3 As String, nmkbn As Enum名称区分, kinoid As String, patternno As Integer) As String
        ...
        ''' 名称取得(条件コード)リスト
        ''' </summary>
        'Public Function GetFreeItemCdNm(db As DaDbContext, nmkbn As Enum名称区分, inputtype As Enum入力タイプ, codejoken1 As String, codejoken2 As String, codejoken3 As String) As String()
        '    Dim strs = New String(2) {}
        '    If Not String.IsNullOrEmpty(codejoken1) Then
        '        'コード条件1(コード：名称)
        '        strs(0) = CmLogicUtil.GetFreeItemCdNm(db, Gjs.Enum名称区分.名称, inputtype, Enum条件コード区分.コード1, codejoken1)
        '        If Not String.IsNullOrEmpty(codejoken2) Then
        '            'コード条件2(コード：名称)
        '            strs(1) = CmLogicUtil.GetFreeItemCdNm(db, Gjs.Enum名称区分.名称, inputtype, Enum条件コード区分.コード2, codejoken2, codejoken1)
        '        End If
        '    End If
        '    Return strs
        'End Function
        ...
        ''' 詳細条件のコントローラータイプを取得
        ''' </summary>
        Public Function GetFreeKoumokuSethanyokbn1(inputtype As Enum入力タイプ, ctrltype As Enumコントローラータイプ) As String
            Select Case ctrltype
                Case Enumコントローラータイプ.通用項目
                    Return DaConvertUtil.EnumToStr(GetSearchSetType(inputtype))
                Case Enumコントローラータイプ.一覧選択
                    Return DaConvertUtil.EnumToStr(GetMstKbn(inputtype))
                Case Enumコントローラータイプ.参照ダイアログ
                    Return DaConvertUtil.EnumToStr(GetSearchDialogSetType(inputtype))
                Case Else
        ...
        Public Function GetKetas(keta As String) As (Integer, Integer)
            ...
            Dim ketas = keta
            Dim keta1 As Integer = If(ketas.Count > 0, DaConvertUtil.CInt(ketas(0)), Nothing)                   '入力桁(整数)
            Dim keta2 As Integer = If(ketas.Count = 2, DaConvertUtil.CInt(ketas(1)), Nothing)                  '入力桁(小数)
        ...
        ''' フリー項目初期設定(共通部分)
        ''' </summary>
        Public Function GetFreeVM(db As DaDbContext, vm As FixFreeItemBaseVM, datatypes As List(Of tm_afmeisyoDto), kbn As Enumフリーマスタ分類, kinoid As String, patternno As Integer, numvalue As Double, strvalue As String, fusyoflg As Boolean, itemcd As String, itemnm As String, groupid2 As String, tani As String, keta As String, hissuflg As Boolean, hanif As String, hanit As String, inputflg As Boolean, msgkbn As EnumMsgCtrlKbn, biko As String, syokiti As String, inputtype As Enum入力タイプ, codejoken1 As String, codejoken2 As String, codejoken3 As String) As FixFreeItemBaseVM
            ...
            vm.hanif = DaConvertUtil.CStr(hanif)         '入力範囲（開始）
            vm.hanit = DaConvertUtil.CStr(hanit)         '入力範囲（終了）
            vm.inputflg = inputflg         '入力フラグ
            vm.msgkbn = msgkbn             'メッセージ区分

            '入力区分関連情報を取得
            '保存型取得
            vm.biko = DaConvertUtil.CStr(biko)           '備考
            '画面項目入力区分
            ...
            ''' 
            '''             //入力区分関連情報を取得
            '''             var nmDto = datatypes.Find(x => x.nmcd == JBD.GJS.DaConvertUtil.EnumToStr(inputtype))!;
            ''' 
            ''' 
            ''' Cannot convert LocalDeclarationStatementSyntax, System.InvalidCastException: 型 'Microsoft.CodeAnalysis.VisualBasic.Syntax.EmptyStatementSyntax' のオブジェクトを型 'Microsoft.CodeAnalysis.VisualBasic.Syntax.ExpressionSyntax' にキャストできません。
            '''    場所 ICSharpCode.CodeConverter.VB.CommonConversions.RemodelVariableDeclaration(VariableDeclarationSyntax declaration)
            ...
            ''' Input:
            '''             //保存型取得
            '''             var savekbn = (JBD.GJS.Service.Enum入力値保存型)JBD.GJS.DaConvertUtil.CInt(nmDto.hanyokbn1!);
            ''' 
            ''' 
            ...
            ''' Input:
            ''' 
            '''             vm.inputkbn = (JBD.GJS.Service.Enum画面項目入力)JBD.GJS.DaConvertUtil.CInt(nmDto.hanyokbn3!)
            ''' 
            ...
            If Not String.IsNullOrEmpty(syokiti) AndAlso vm.inputkbn = Enum画面項目入力.選択 Then
                ...
                ''' 
                ''' Input:
                '''                 vm.syokiti = JBD.GJS.Service.CmLogicUtil.GetFreeItemCdNm(db, syokiti!, kbn, inputtype, codejoken1, codejoken2, codejoken3, JBD.GJS.Enum名称区分.名称, kinoid, patternno)
                ''' 
            Else
                vm.syokiti = DaConvertUtil.CStr(syokiti)
            End If
            ...
            '                ''' 
            '                ''' Input:
            '                '''                             vm.value = JBD.GJS.Service.CmLogicUtil.GetFreeItemCdNm(db, strvalue!, kbn, inputtype, codejoken1, codejoken2, codejoken3, JBD.GJS.Enum名称区分.名称, kinoid, patternno)
            '                ''' 
            '            End If
            ...
            '一覧選択リスト
            vm.cdlist = New List(Of DaSelectorModel)()
            ...
            If vm.inputkbn = Enum画面項目入力.選択_検索 Then
                ...
                vm.cdlist = CmLogicUtil.GetCdList(db, kinoid, DaConvertUtil.CNStr(patternno), kbn2)
            End If
        ...
        ''' 一覧選択リスト(項目)
        ''' </summary>
        Public Function GetFreeItemCdList(db As DaDbContext, nmkbn As Enum名称区分, freemstkbn As Enumフリーマスタ分類, inputtype As Enum入力タイプ, codejoken1 As String, codejoken2 As String, codejoken3 As String, kinoid As String, patternno As Integer) As List(Of DaSelectorModel)
            ...
            Select Case kbn
                Case Enumマスタ区分.名称マスタ, Enumマスタ区分.汎用マスタ
                    Return GetCommonCdList(db, kbn, nmkbn, Nothing)
                Case Enumマスタ区分.ユーザーマスタ, Enumマスタ区分.地区情報マスタ
                    Return CmLogicUtil.GetCommonCdList(db, kbn, nmkbn, Nothing, DaConvertUtil.CStr(codejoken1))
                Case Enumマスタ区分.所属グループマスタ, Enumマスタ区分.会場情報マスタ
                    Return GetCommonCdList(db, kbn, nmkbn, Nothing)
                Case Enumマスタ区分.医療機関マスタ, Enumマスタ区分.医療機関マスタ_保険
        ...
        ''' 名称取得(条件コード)
        ''' </summary>
        'Private Function GetFreeItemCdNm(db As DaDbContext, nmkbn As Enum名称区分, inputtype As Enum入力タイプ, kbn As Enum条件コード区分, cd As String, ParamArray keys As String()) As String
        ...
        ''' 行政区名称取得
        ''' </summary>
        'Private Function GetGyoseiku(kbn As Enum地区区分, dtl As List(Of tm_aftikuDto), code As String) As String
        ...
        ''' ログイン時権限データ取得
        ''' </summary>
        Private Function GetDto(db As DaDbContext, token As String, userid As String, regsisyo As String) As tt_aftokenDto
            Dim tokenseq = CmTokenService.GetTokenUD(token, userid, regsisyo)
            Dim dto As tt_aftokenDto = New tt_aftokenDto()
            Return dto
            'return tt_aftoken.GetDtoByKey(tokenseq);
        End Function
        ''' <summary>
        ''' 画面権限一覧取得
        ''' </summary>
        Private Function GetAuthList(dto As tt_aftokenDto) As List(Of GamenModel)
        ...
        ''' 更新可能支所一覧取得
        ''' </summary>
        Private Function GetSisyoList(dto As tt_aftokenDto) As List(Of String)
        ...
        ''' 個人番号権限フラグ取得
        ''' </summary>
        Private Function GetPersonalflg(list As List(Of GamenModel), kinoid As String) As Boolean
        ...
        ''' 集約コード取得(事業コード)
        ''' </summary>
        Private Function GetCodeSet(db As DaDbContext, code As String, kbn As EnumHanyoKbn) As HashSet(Of String)
        ...
        ''' マスタ区分取得
        ''' </summary>
        Private Function GetMstKbn(inputtype As Enum入力タイプ) As Enumマスタ区分
            Select Case inputtype
        ...
        ''' 参照ダイアログ区分取得
        ''' </summary>
        Private Function GetSearchDialogSetType(inputtype As Enum入力タイプ) As Enum参照ダイアログ項目区分
            Select Case inputtype
                Case Enum入力タイプ.宛名番号
        ...
        Private Function GetCommonCdList(db As DaDbContext, kbn As Enumマスタ区分, nmkbn As Enum名称区分, keyList As List(Of String), ParamArray keys As String()) As List(Of DaSelectorModel)
            Select Case kbn
                ...
                'Case Gjs.Enumマスタ区分.ユーザーマスタ, Gjs.Enumマスタ区分.地区情報マスタ
                ...
                'Case Gjs.Enumマスタ区分.医療機関マスタ, Gjs.Enumマスタ区分.医療機関マスタ_保険, Gjs.Enumマスタ区分.事業従事者担当者情報マスタ
                '    If keyList IsNot Nothing Then
                '        Return Gjs.DaSelectorService.GetSelectorList(db, nmkbn, kbn, False, Gjs.DaConvertUtil.CStr(Gjs.DaConvertUtil.ListToCommaStr(keyList)))
                '    End If
                '    'todo
                '    Return DaSelectorServiceHelpers.GetSelectorList(db, nmkbn, kbn)
                Case Else
        ...
        ''' 詳細条件項目区分取得
        ''' </summary>
        Private Function GetSearchSetType(inputtype As Enum入力タイプ) As Enum項目区分
            Select Case inputtype
                Case Enum入力タイプ.数値_整数, Enum入力タイプ.数値_小数点付き実数, Enum入力タイプ.数値_符号付き整数
                    Return Enum項目区分.数字
                Case Enum入力タイプ.半角文字_半角数字, Enum入力タイプ.半角文字_半角英数字, Enum入力タイプ.半角文字_年, Enum入力タイプ.半角文字_年_不詳あり, Enum入力タイプ.半角文字_年月, Enum入力タイプ.半角文字_年月_不詳あり, Enum入力タイプ.半角文字_時刻, Enum入力タイプ.半角文字_半角, Enum入力タイプ.全角文字_全角_改行不可, Enum入力タイプ.全角文字_全角_改行可, Enum入力タイプ.全角半角文字_全角半角_改行不可, Enum入力タイプ.全角半角文字_全角半角_改行可
                    Return Enum項目区分.文字
        ...
        Public Function GetCdList(db As DaDbContext, kinoid As String, patternno As String, kbn As Enum参照ダイアログ項目区分, Optional hasStopFlg As Boolean = False) As List(Of DaSelectorModel)
            ...
            If kbn = Enum参照ダイアログ項目区分.医療機関 OrElse kbn = Enum参照ダイアログ項目区分.事業従事者 OrElse kbn = Enum参照ダイアログ項目区分.検診実施機関 Then
                ...
                ''' 
                ''' Input:
                '''                 keyList = JBD.GJS.Service.CmLogicUtil.GetJigyocdList(db, kbn == JBD.GJS.Enum参照ダイアログ項目区分.事業従事者 
                '''                                         JBD.GJS.Service.Enum事業コード区分.事業従事者 : JBD.GJS.Service.Enum事業コード区分.医療機関, kinoid!, patternno, false)
                ''' 

                '参照画面項目一覧取得(詳細条件)
                'Return GetSearchItemCdList(db, kbn, Gjs.Enum名称区分.名称, keyList, hasStopFlg)
            End If
        ...
        Public Function GetUserRyoikiMaxCd(keta As Integer) As Integer
            ...
            If DaConvertUtil.CInt(keta) > 0 Then
                maxCd = CInt(Math.Pow(10, DaConvertUtil.CInt(keta))) - 1
            Else
