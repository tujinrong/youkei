...
#Region "***変数定義***"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Property p8090_KI As Integer
    Property p8090_KEIYAKUSYA_CD As Integer
    Property p8090_KEIYAKUSYA_NM As String
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Property paryKEY_8090 As New List(Of frmGJ8090.T_KEY)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Property pCurrentKey As frmGJ8090.T_KEY
        ...
        Try
            ...
            Select Case pSyoriKbn
                Case Enu_SyoriKubun.Insert       '新規入力
                    ...
                    lblTotal.Text = ""
                    '★初期コントロールにフォーカスセット
                    txt_NOJO_CD.Select()         '2014/05/12　修正
                ...
                Case Enu_SyoriKubun.Update       '変更
                    ...
                    '行遷移
                    lblTotal.Text = paryKEY_8090.Count
        ...
        Try
            ...
            Select Case pSyoriKbn
                Case Enu_SyoriKubun.Insert       '新規入力
                    ...
                    pCurrentKey.NOJO_CD = txt_NOJO_CD.Text.Trim
...
#Region "cmdEnd_Click 終了ボタンクリック処理"
    ...
    Private Sub cmdEnd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdEnd.Click
        Try
            ...
            DirectCast(Owner, frmGJ8090).pSel_POS = pSel_NO - 1
...
#Region "契約者農場 Validating"
    '------------------------------------------------------------------
    'プロシージャ名  :txt_SEQNO_Validating
    '説明            :契約者農場 Validating
    '------------------------------------------------------------------
    Private Sub txt_SEQNO_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txt_NOJO_CD.Validating
...
#Region "f_Data_Insert 契約者農場更新処理"
    ...
    Private Function f_Data_Update() As Boolean
        ...
        Try
            ...
            '期 
            Cmd.Parameters.Add("IN_KI", p8090_KI)
            '契約者番号
            Cmd.Parameters.Add("IN_KEIYAKUSYA_CD", p8090_KEIYAKUSYA_CD)
            '農場番号 
...
#Region "f_ObjectClear 画面クリア処理"
    ...
    Private Function f_ObjectClear(ByVal wKbn As String) As Boolean
        ...
        Try
            ...
            'ヘッダ表示
            lbl_KI.Text = "第" & p8090_KI & "期"
            lbl_KEIYAKUSYA_CD.Text = p8090_KEIYAKUSYA_CD
            lbl_KEIYAKUSYA_NM.Text = p8090_KEIYAKUSYA_NM
    ...
    Private Function f_ExsistData() As Boolean
        ...
        Try
            ...
            wSql = "SELECT * FROM TM_KEIYAKU_NOJO" & _
                    " WHERE KI = " & p8090_KI & _
                    "  AND KEIYAKUSYA_CD = " & p8090_KEIYAKUSYA_CD & _
...
#Region "f_SetForm_Data 初期画面表示(修正)処理"
    ...
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Private Function f_SetForm_Data(ByVal wkKey As frmGJ8090.T_KEY) As Boolean
        Dim ret As Boolean = False
        Dim wkDS As New DataSet
        Dim wSql As String = String.Empty
        ...
        Try
            ...
            wSql = "SELECT * FROM TM_KEIYAKU_NOJO" & _
                    " WHERE KI = " & p8090_KI & _
                    "  AND KEIYAKUSYA_CD = " & p8090_KEIYAKUSYA_CD & _
                    "  AND NOJO_CD = " & pCurrentKey.NOJO_CD
            ...
            With wkDS.Tables(0)
                ...
                '農場
                txt_NOJO_CD.Text = pCurrentKey.NOJO_CD
                '農場名(正式)
                txt_NOJO_NAME.Text = WordHenkan("N", "S", .Rows(0)("NOJO_NAME"))
                ...
                '明細番号
                txt_MEISAI_NO.Text = WordHenkan("N", "S", .Rows(0)("MEISAI_NO"))
...
#Region "f_RecValidating 行遷移処理"
    ...
    Private Function f_RecValidating(ByVal intRecNo As Integer) As Boolean
        ...
        Try
            '★KEY取得
            Dim tKEY As New frmGJ8090.T_KEY
            tKEY = paryKEY_8090.Item(intRecNo)
            pCurrentKey = tKEY
...
#End Region
    ...
    Private Sub txt_Now_TextChanged(sender As Object, e As EventArgs) Handles txt_Now.TextChanged
