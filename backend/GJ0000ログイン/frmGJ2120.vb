...
'*　　①　フォーム名　　　  frmGJ2120.vb
'*
'*　　②　機能概要　　　　　家畜防疫互助金積立金等請求督促通知書（新規加入者）発行処理
'*
'*　　③　作成日　　　　　　2015/03/26  BY JBD
'*
'*　　④　更新日            
'*
'*******************************************************************************
...
Public Class frmGJ2120

#Region "*** 変数定義 ***"

    Public pRptName As String = "生産者積立金等請求督促通知書（新規加入）"     '帳票名

    Private pInitKI As Integer              '期(初期値)
    Private pEnterKI As String              '期(変更前)
    Private pKI As String                   '期
    Private pSEIKYU_KAISU_F As String       '請求回数
    Private pSEIKYU_KAISU_T As String       '請求回数
...
#Region "frmGJ2120_Load Form_Loadイベント"

    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ2120_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ2120_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
...
#Region "処理区分　Chanegeイベント"
    ...
    Private Sub rbtn_SYORI_KBN_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
        ...
        If (sender.Equals(rbtn_SYORI_KBN1) AndAlso rbtn_SYORI_KBN1.Checked) OrElse _
           (sender.Equals(rbtn_SYORI_KBN2) AndAlso rbtn_SYORI_KBN2.Checked) OrElse _
           (sender.Equals(rbtn_SYORI_KBN3) AndAlso rbtn_SYORI_KBN3.Checked) Then
            '画面再表示
            If txt_KI.Text.Trim = "" Then
                '期：未入力
                f_Rireki_Set(0)
            ElseIf txt_SEIKYU_KAISU_F.Text.Trim = "" Then
                '期：入力　回数：未入力
                f_Rireki_Set(1)
            Else
                '期：入力　回数：入力
...
#Region "請求回数"
    '------------------------------------------------------------------
    'プロシージャ名  :txt_SEIKYU_KAISU_F_Validating
    '説明            :請求回数FROM Validatingイベント
    '------------------------------------------------------------------
    Private Sub txt_SEIKYU_KAISU_F_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txt_SEIKYU_KAISU_F.Validating

        Try

            Call s_From_Validating(txt_SEIKYU_KAISU_F, txt_SEIKYU_KAISU_T)

            '期・回数　変更時、再表示
            If pSEIKYU_KAISU_F <> txt_SEIKYU_KAISU_F.Text Then
                '画面表示
                f_Rireki_Set(2)
            Else
                '入金期限が空白のとき、入金期限のみ再表示
    ...
    '------------------------------------------------------------------
    'プロシージャ名  :txt_SEIKYU_KAISU_T_Validating
    '説明            :請求回数TO Validatingイベント
    '------------------------------------------------------------------
    Private Sub txt_SEIKYU_KAISU_T_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txt_SEIKYU_KAISU_T.Validating

        Try
            Call s_To_Validating(txt_SEIKYU_KAISU_T, txt_SEIKYU_KAISU_F)

            '期・回数　変更時、再表示
            If pSEIKYU_KAISU_T <> txt_SEIKYU_KAISU_T.Text Then
                '画面表示
                f_Rireki_Set(2)
            Else
                '入金期限が空白のとき、入金期限のみ再表示
...
#Region "f_Save 請求入金更新"
    ...
    Private Function f_Save() As Boolean
        ...
        Try
            ...
            '処理区分
            Cmd.CommandText = "PKG_GJ2120.GJ2120_TOKUSOKU"
            '--------------------
            '請求
            '--------------------
            '期
            ...
            '回数
            Dim paraSEIKYU_KAISU_F As OracleParameter = Cmd.Parameters.Add("IN_SEIKYU_KAISU_F", CInt(txt_SEIKYU_KAISU_F.Text))
            Dim paraSEIKYU_KAISU_T As OracleParameter = Cmd.Parameters.Add("IN_SEIKYU_KAISU_T", CInt(txt_SEIKYU_KAISU_T.Text))
            '納付期限
            ...
            Else
                Dim paraHAKKO_DATE As OracleParameter = Cmd.Parameters.Add("IN_SEIKYU_HAKKO_DATE", f_DateTrim(date_HAKKO_DATE.Value))
            End If
            '発行年
...
#Region "f_Rireki_Set 請求情報セット"
    ...
    Public Function f_Rireki_Set(ByVal wKbn As Integer) As Boolean
        ...
        Try
            ...
            '回数　未入力
            If txt_SEIKYU_KAISU_F.Text = "" Then
                pSEIKYU_KAISU_F = 0
            Else
                pSEIKYU_KAISU_F = txt_SEIKYU_KAISU_F.Text
            End If
            If txt_SEIKYU_KAISU_T.Text = "" Then
                pSEIKYU_KAISU_T = 0
            Else
                pSEIKYU_KAISU_T = txt_SEIKYU_KAISU_T.Text
            End If
            ...
            Select Case wKbn
                Case 2
                    '回数入力
                    wSql = " SELECT"
                    wSql += " MIN(SEIKYU_DATE) OVER() SEIKYU_DATE_MIN, "
                    wSql += " MAX(SEIKYU_DATE) OVER() SEIKYU_DATE_MAX, "
                    wSql += " MIN(KIGEN_DATE)  OVER() KIGEN_DATE_MIN, "
                    wSql += " MAX(KIGEN_DATE)  OVER() KIGEN_DATE_MAX, "
                    wSql += " KEI.* "
                    wSql += " FROM TT_TUMITATE_KEISAN KEI"
                    wSql += " WHERE KI = " & pKI
                    wSql += " 　AND SEIKYU_KAISU >= " & pSEIKYU_KAISU_F
                    wSql += " 　AND SEIKYU_KAISU <= " & pSEIKYU_KAISU_T
            ...
            With wDS.Tables(0)
                If .Rows.Count = 0 Then
                    ...
                    '画面クリア
                    If wKbn <> 2 Then
                        txt_SEIKYU_KAISU_F.Text = ""
                        txt_SEIKYU_KAISU_T.Text = ""
                    End If
                ...
                Else
                    ...
                    '回数
                    If wKbn <> 2 Then
                        pSEIKYU_KAISU_F = CInt(WordHenkan("N", "Z", .Rows(0)("SEIKYU_KAISU")))
                        pSEIKYU_KAISU_T = CInt(WordHenkan("N", "Z", .Rows(0)("SEIKYU_KAISU")))
                        txt_SEIKYU_KAISU_F.Text = CInt(WordHenkan("N", "Z", .Rows(0)("SEIKYU_KAISU")))
                        txt_SEIKYU_KAISU_T.Text = CInt(WordHenkan("N", "Z", .Rows(0)("SEIKYU_KAISU")))
                    End If
                    '請求日
                    If wKbn < 2 Then
                        '期入力
                    ...
                    Else
                        '回数入力
                        If WordHenkan("N", "S", .Rows(0)("SEIKYU_DATE_MIN")) <> WordHenkan("N", "S", .Rows(0)("SEIKYU_DATE_MAX")) Then
                            '請求日が複数のとき表示しない
                            date_SEIKYU_DATE.Value = Nothing
                        Else
                            date_SEIKYU_DATE.Value = CDate(WordHenkan("N", "S", .Rows(0)("SEIKYU_DATE")))
                    ...
                    If rbtn_SYORI_KBN2.Checked Then
                    Else
                        If WordHenkan("N", "S", .Rows(0)("KIGEN_DATE_MIN")) <> WordHenkan("N", "S", .Rows(0)("KIGEN_DATE_MAX")) Then
                            '入金期限が複数のとき表示しない
                            date_KIGEN_DATE.Value = Nothing
                        Else
                            If WordHenkan("N", "S", .Rows(0)("KIGEN_DATE_MAX")) = "" Then
                                date_KIGEN_DATE.Value = Nothing
                            Else
                                date_KIGEN_DATE.Value = CDate(WordHenkan("N", "S", .Rows(0)("KIGEN_DATE_MAX")))
                            End If
                        End If
    ...
    Private Function f_Input_Check(ByRef wSEIKYUSU As Integer) As Boolean
        ...
        Try
            ...
            If txt_SEIKYU_KAISU_F.Text = "" Then
                ...
                txt_SEIKYU_KAISU_F.Focus()
                Exit Try
            End If
            If txt_SEIKYU_KAISU_T.Text = "" Then
                Show_MessageBox_Add("W008", "回数")      '@0は必須入力項目です。
                txt_SEIKYU_KAISU_T.Focus()
                Exit Try
            End If
            ...
            '--------------------------------------------------
            '回数
            If Not f_Daisyo_Check(txt_SEIKYU_KAISU_F, txt_SEIKYU_KAISU_T, "請求回数", True, 3) Then
                Exit Try
            End If
            '事務委託先
            ...
            '請求計算履歴　読込
            wSql = " SELECT"
            wSql &= " MIN(TUM.KEIYAKU_HENKO_KBN) OVER() KEIYAKU_HENKO_KBN_MIN, "
            wSql &= " MAX(TUM.KEIYAKU_HENKO_KBN) OVER() KEIYAKU_HENKO_KBN_MAX, "
            wSql &= " TUM.* "
            wSql &= " FROM TT_TUMITATE TUM"
            wSql &= " WHERE KI = " & txt_KI.Text.Trim
            wSql &= "   AND TUM.SEIKYU_KAISU >= " & txt_SEIKYU_KAISU_F.Text.Trim
            wSql &= "   AND TUM.SEIKYU_KAISU <= " & txt_SEIKYU_KAISU_T.Text.Trim
            ...
            If wDS.Tables(0).Rows.Count = 0 Then
                ...
                Exit Try
            Else
                If WordHenkan("N", "Z", wDS.Tables(0).Rows(0)("KEIYAKU_HENKO_KBN_MIN")) <> "0" Then
                ...
                If .Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        If Show_MessageBox_Add("Q012",
                           ...
                           "生産者積立金等請求書通知書(新規登録)(督促状)を" & vbCrLf & "発行してもよろしいですか。") = System.Windows.Forms.DialogResult.No Then
                            Exit Try
                        End If
                        'End If
...
#Region "f_Where_Set 条件セット処理"
    ...
    Private Function f_Where_Set(ByVal wKBN As Integer, ByRef wWHERE As String) As Boolean
        ...
        Try
            ...
            wWHERE &= "  AND TUM.SEIKYU_KAISU >= " & txt_SEIKYU_KAISU_F.Text.Trim
            wWHERE &= "  AND TUM.SEIKYU_KAISU <= " & txt_SEIKYU_KAISU_T.Text.Trim
            wWHERE &= "  AND TUM.TUMITATE_KBN = 1"

            '契約変更区分:0(新規のみ対象)
            wWHERE &= "  AND KEIYAKU_HENKO_KBN = 0"
            '変更請求区分:1(請求のみ対象)
            wWHERE &= "  AND SEIKYU_HENKAN_KBN = 1"
            ...
            If rbtn_SYORI_KBN1.Checked Then
                '初回発行
                wWHERE &= "  AND (TUM.NOFUKIGEN_DATE <= TO_DATE('" & Format(Now, "yyyy/MM/dd") & "','YYYY/MM/DD') OR " & _
                                 "TUM.NOFUKIGEN_DATE <= TO_DATE('" & Format(date_HAKKO_DATE.Value, "yyyy/MM/dd") & "','YYYY/MM/DD'))"
                wWHERE &= "  AND TUM.SYORI_JOKYO_KBN = 1"           '請求中
                wWHERE &= "  AND TUM.TOKUSOKU_HAKKO_DATE IS NULL"
            Else
                wWHERE &= "  AND TUM.SYORI_JOKYO_KBN = 2"
                wWHERE &= "  AND TUM.TOKUSOKU_HAKKO_DATE IS NOT NULL"
            End If
        ...
        Try
            ...
            If wkDSRep.Tables(0).Rows.Count > 0 Then
                ...
                Else
                    pFlag = 3
                End If
                Dim w As New rptGJ2120
                w.sub1(wkDSRep, pFlag)

                '    'ヘッダ用の値を保管
                '    wkAR.pKikin2 = pKikin2      '2017/07/19　追加
                '    ' 用紙サイズを A4縦 に設定します。
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSql As String) As Boolean
        ...
        Try
            ...
            'wSql &= "  M01.RYAKUSYO KEIYAKU_KBN_NM,"   '2017/07/19　修正
            wSql &= "  M01.MEISYO KEIYAKU_KBN_NM,"      '2017/07/19　修正
            '  --協会マスタ
            ...
            If rbtn_SYORI_KBN1.Checked OrElse rbtn_SYORI_KBN3.Checked Then
                ...
                wSql &= "  TO_CHAR(TO_DATE('" & Format(date_HAKKO_DATE.Value, "yyyy/MM/dd") & "'), 'EEYY""年""MM""月""DD""日""', 'NLS_CALENDAR=''JAPANESE IMPERIAL''') AS SEIKYU_HAKKO_DATE,"
            ...
            End If
            '2017/07/19　修正開始
            'wSql &= "  WK1.HASU1,  WK1.TUMITATE_TANKA1,  WK1.TUMITATE_KIN1,"
            'wSql &= "  WK1.HASU2,  WK1.TUMITATE_TANKA2,  WK1.TUMITATE_KIN2,"
            'wSql &= "  WK1.HASU3,  WK1.TUMITATE_TANKA3,  WK1.TUMITATE_KIN3,"
            'wSql &= "  WK1.HASU4,  WK1.TUMITATE_TANKA4,  WK1.TUMITATE_KIN4,"
            'wSql &= "  WK1.HASU5,  WK1.TUMITATE_TANKA5,  WK1.TUMITATE_KIN5,"
            'wSql &= "  WK1.HASU6,  WK1.TUMITATE_TANKA6,  WK1.TUMITATE_KIN6,"
            For i = 1 To 6
                wSql &= "  WK1.TORI_KBN_NM" & i & "," & _
                        "  WK1.HASU" & i & "," & _
                        "  WK1.TUMITATE_TANKA" & i & "," & _
                        "  WK1.TUMITATE_KIN" & i & ", "
            Next
            '2017/07/19　修正終了
            ...
            For i = 1 To 6
                wSql &= "     MAX(CASE WHEN WK2.GYO = " & i & " THEN WK2.TORI_KBN_NM    ELSE NULL END)  TORI_KBN_NM" & i & ","     '2017/07/19　追加
            ...
            wSql &= "        MAX(MEI.TUMITATE_KIN) TUMITATE_KIN"
            wSql &= "      FROM"
            wSql &= "        TT_TUMITATE TUM,"
            wSql &= "        TM_KEIYAKU  KYK,"
...
#End Region
    ...
    Private Sub txt_SEIKYU_KAISU_F_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_SEIKYU_KAISU_F.TextChanged
