...
'*　　①　フォーム名　　　  frmGJ2070.vb
'*
'*　　②　機能概要　　　　　家畜防疫互助金積立金等請求通知書（新規加入者）発行処理
'*
'*　　③　作成日　　　　　　2015/03/23  BY JBD
'*
'*　　④　更新日            
'*
'*******************************************************************************
...
#Region "frmGJ2070_Load Form_Loadイベント"

    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ2070_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ2070_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
...
#Region "cmdPreview_Click プレビューボタンクリック処理"
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        ...
        Try
            ...
            '請求入金更新
            If rbtn_SYORI_KBN4.Checked = False Then '2020/08/07 JBD9020 ADD
                If Not f_Save() Then
                    Exit Try
                End If
            End If '2020/08/07 JBD9020 ADD
...
#Region "処理区分　Chanegeイベント"
    ...
    Private Sub rbtn_SYORI_KBN_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
                rbtn_SYORI_KBN1.CheckedChanged, rbtn_SYORI_KBN2.CheckedChanged, rbtn_SYORI_KBN3.CheckedChanged, rbtn_SYORI_KBN4.CheckedChanged '2020/08/07 JBD9020 処理区分追加

        If pJump Then
            Exit Sub
        End If

        '2020/08/07 JBD9020 UPD START
        ...
        If (sender.Equals(rbtn_SYORI_KBN1) AndAlso rbtn_SYORI_KBN1.Checked) OrElse
           ...
           (sender.Equals(rbtn_SYORI_KBN4) AndAlso rbtn_SYORI_KBN4.Checked) Then
            '2020/08/07 JBD9020 UPD START
            '画面再表示
...
#Region "f_Save 請求入金更新"
    ...
    Private Function f_Save() As Boolean
        ...
        Try
            ...
            '処理区分
            Cmd.CommandText = "PKG_GJ2070.GJ2070_SEIKYU"
            '--------------------
            '請求
            '--------------------
            '期
...
#Region "f_Rireki_Set 請求情報セット"
    ...
    Public Function f_Rireki_Set(ByVal wKbn As Integer) As Boolean
        ...
        Try
            ...
            '2020/08/07 JBD9020 UPD START
            'If rbtn_SYORI_KBN2.Checked Then
            If rbtn_SYORI_KBN2.Checked Or rbtn_SYORI_KBN4.Checked Then
                '2020/08/07 JBD9020 UPD END
                date_KIGEN_DATE.Text = ""
                ...
                If .Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        If Show_MessageBox_Add("Q012",
                           ...
                           "生産者積立金等請求通知書(新規加入)を" & vbCrLf & "発行してもよろしいですか。") = System.Windows.Forms.DialogResult.No Then
                            Exit Try
                        End If
                        'End If
...
#Region "f_Where_Set 条件セット処理"
    ...
    Private Function f_Where_Set(ByVal wKBN As Integer, ByRef wWHERE As String) As Boolean
        ...
        Try
            ...
            '2020/08/07 JBD9020 UPD START
            'If rbtn_SYORI_KBN1.Checked Then
            If rbtn_SYORI_KBN1.Checked Or rbtn_SYORI_KBN4.Checked Then
                '2020/08/07 JBD9020 UPD END
        ...
        Try
            ...
            If wkDSRep.Tables(0).Rows.Count > 0 Then
                ...
                ElseIf rbtn_SYORI_KBN4.Checked Then '2020/08/07 JBD9020 ADD 
                    pSyoriKbn = 4              '2020/08/07 JBD9020 ADD 
                Else
                    pSyoriKbn = 3
                End If
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSql As String) As Boolean
        ...
        Try
            ...
            wSql &= "  SELECT"
            wSql &= "  WK1.KI,"
            wSql &= "  '内　'||WK1.TESURYO_SYOHIZEI_RITU || '％消費税' TESURYO_SYOHIZEI_RITU,"    '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "  WK1.TESURYO_SYOHIZEI TESURYO_SYOHIZEI,"                                   '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "  WK1.SAGAKU_TESURYO_SYOHIZEI SAGAKU_TESURYO_SYOHIZEI,"                     '2023/08/10　追加 インボイス対応　JBD453
            ...
            wSql &= "  '(登録番号：' || KYO.TOUROKU_NO  || ')' TOUROKU_NO," '2023/08/10　追加 インボイス対応　JBD453
            ...
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"     '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"               '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI," '2023/08/10　追加 インボイス対応　JBD453
            ...
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"     '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"               '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "        MAX(TUM.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI," '2023/08/10　追加 インボイス対応　JBD453
            wSql &= "        MEI.TORI_KBN,"
