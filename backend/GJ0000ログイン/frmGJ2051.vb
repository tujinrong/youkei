...
'*　　①　フォーム名　　　  frmGJ2051.vb
'*
'*　　②　機能概要　　　　　（相殺対応）互助金生産者積立金等請求書兼返還通知書（返還）発行処理
'*
'*　　③　作成日　　　　　　2018/06/08  BY JBD
'*
'*　　④　更新日            
'*
'*******************************************************************************
...
Public Class frmGJ2051

#Region "*** 変数定義 ***"

    Public pRptName As String = "生産者積立金等請求書兼返還金通知書（一部返還）"     '帳票名
...
#Region "frmGJ2051_Load Form_Loadイベント"

    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ2051_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ2051_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
...
#Region "f_Save 請求入金更新"
    ...
    Private Function f_Save() As Boolean
        ...
        Try
            ...
            '処理区分
            Cmd.CommandText = "PKG_GJ2050.GJ2050_SEIKYU"
            '--------------------
            '請求
            '--------------------
            '期
        ...
        Try
            ...
            With wDS.Tables(0)
                ...
                Else
                    ...
                    If rbtn_SYORI_KBN2.Checked Then
                    Else
                        If WordHenkan("N", "S", .Rows(0)("FURIKOMI_YOTEI_DATE")) = "" Then
                            date_KIGEN_DATE.Text = ""
                        Else
                            date_KIGEN_DATE.Value = CDate(WordHenkan("N", "S", .Rows(0)("FURIKOMI_YOTEI_DATE")))
                        End If
                    End If
                End If
    ...
    Private Function f_Input_Check(ByRef wSEIKYUSU As Integer) As Boolean
        ...
        Try
            ...
            Else
                '返還期限
                If date_KIGEN_DATE.Value Is Nothing Then
                    Show_MessageBox_Add("W008", "返還期限")      '@0は必須入力項目です。
                ...
                If .Rows.Count > 0 Then
                    ...
                    Else
                        If Show_MessageBox_Add("Q012",
                           "返還対象人数 : " & Format(wSEIKYUSU, "##,##0") & "人" & vbCrLf &
                           "返還金額 : " & Format(wKINGAK, "#,###,###,##0") & "円" & vbCrLf &
                           "生産者積立金等請求書兼返還通知書(返還)を" & vbCrLf & "発行してもよろしいですか。") = System.Windows.Forms.DialogResult.No Then
                            Exit Try
                        End If
                    End If
                Else
...
#Region "f_Where_Set 条件セット処理"
    ...
    Private Function f_Where_Set(ByVal wKBN As Integer, ByRef wWHERE As String) As Boolean
        ...
        Try
            ...
            '変更請求区分:3(請求兼返還(返還)のみ対象)
            wWHERE &= "  AND SEIKYU_HENKAN_KBN = 3"
            '契約変更区分:0(新規のみ対象)
        ...
        Try
            ...
            If wkDSRep.Tables(0).Rows.Count > 0 Then
                ...
                Else
                    psyorikbn = 3
                End If
                Dim w As New rptGJ2051
                w.sub1(wkDSRep, psyorikbn)

                '    '印影を取得
                '    wkAR.imgInei = f_inei_Get()
                '    'ヘッダ用の値を保管
                '    wkAR.pKikin2 = pKikin2
                '    ' 用紙サイズを A4縦 に設定します。
...
#Region "f_make_SQL 帳票データ出力用ＳＱＬ作成"
    ...
    Private Function f_make_SQL(ByRef wSql As String) As Boolean
        ...
        Try
            ...
            wSql &= "SELECT"
            wSql &= "  WK1.KI,"
            wSql &= "  '内　'||WK1.TESURYO_SYOHIZEI_RITU || '％消費税' TESURYO_SYOHIZEI_RITU,"    '2023/08/21　追加 インボイス対応　JBD453
            wSql &= "  WK1.TESURYO_SYOHIZEI TESURYO_SYOHIZEI,"                                    '2023/08/21　追加 インボイス対応　JBD453
            wSql &= "  WK1.SAGAKU_TESURYO_SYOHIZEI SAGAKU_TESURYO_SYOHIZEI,"                      '2023/08/21　追加 インボイス対応　JBD453
            ...
            wSql &= "  CASE WHEN KYK.DAIHYO_NAME IS NULL THEN NULL ELSE '　' || KYK.DAIHYO_NAME || '様' END KYK_DAIHYO_NAME,"
            wSql &= "  '(' || TRIM(TO_CHAR(WK1.KEIYAKUSYA_CD,'00000')) || ')' KEIYAKUSYA_CD_KAKKO,"
            wSql &= "  WK1.KEIYAKU_KBN,"
            wSql &= "  M01.MEISYO KEIYAKU_KBN_NM,"
            '  --協会マスタ
            wSql &= "  '(登録番号：' || KYO.TOUROKU_NO  || ')' TOUROKU_NO," '2023/08/21　追加 インボイス対応　JBD453
            ...
            wSql &= "  TO_MULTI_BYTE(KYK.FURI_KOZA_NO) FURI_KOZA_NO_N,"
            wSql &= "  KYK.FURI_KOZA_MEIGI,"
            '  --前期積立金返還
            wSql &= "  ZEN.TUMITATE_KIN_KEI,"
            wSql &= "  ZEN.HENKAN_KEISU,"
            wSql &= "  ZEN.HENKAN_KAKUTEI_KIN,"
            wSql &= "  ZEN.TUMITATE_KIN_KEI_1,"
            wSql &= "  ZEN.TUMITATE_KIN_KEI_2,"
            wSql &= "  ZEN.HENKAN_KEISU_1,"
            wSql &= "  ZEN.HENKAN_KEISU_2,"
            wSql &= "  ZEN.HENKAN_KAKUTEI_KIN_1,"
            wSql &= "  ZEN.HENKAN_KAKUTEI_KIN_2,"
            '  --積立情報
            ...
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"        '2023/08/21　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"                  '2023/08/21　追加 インボイス対応　JBD453
            wSql &= "     MAX(WK2.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI,"    '2023/08/21　追加 インボイス対応　JBD453
            ...
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI_RITU) TESURYO_SYOHIZEI_RITU,"          '2023/08/21 インボイス対応 JBD453　追加
            wSql &= "        MAX(TUM.TESURYO_SYOHIZEI) TESURYO_SYOHIZEI,"                    '2023/08/21 インボイス対応 JBD453　追加
            wSql &= "        MAX(TUM.SAGAKU_TESURYO_SYOHIZEI) SAGAKU_TESURYO_SYOHIZEI,"      '2023/08/21 インボイス対応 JBD453　追加
            wSql &= "        MEI.TORI_KBN,"
            ...
            '  --銀行
            wSql &= "  AND KYK.FURI_BANK_CD = BNK.BANK_CD(+)"
            '  --支店
            wSql &= "  AND KYK.FURI_BANK_CD = SIT.BANK_CD(+)"
            wSql &= "  AND KYK.FURI_BANK_SITEN_CD = SIT.SITEN_CD(+)"
            '  --コードマスタ(口座)
            wSql &= "  AND 4 = M04.SYURUI_KBN(+)"
            wSql &= "  AND KYK.FURI_KOZA_SYUBETU = M04.MEISYO_CD(+)"
            '  --前期積立金返還
            wSql &= "  AND WK1.KI = ZEN.KI(+)"
            wSql &= "  AND WK1.KEIYAKUSYA_CD = ZEN.KEIYAKUSYA_CD(+) "
            '--出力順
            wSql &= "ORDER BY"
            wSql &= "  WK1.KEIYAKUSYA_CD"
