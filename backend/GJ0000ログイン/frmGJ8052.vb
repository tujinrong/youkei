...
#Region "*** 変数定義 ***"
    ...
    Public pSel_SITEN_BANKCD As String = String.Empty         '変更時、初期表示番号
...
#Region "*** 画面制御関連 ***"
#Region "frmGJ8052_Load Form_Loadイベント"
    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ8052_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ8052_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
        Try
            'カーソルを砂時計にする
            ...
            Select Case pSyoriKbn
                Case C_INSERT       '新規入力
                    ...
                    '画面内容をセット
                    txt_BANK_CD.Text = pSel_SITEN_BANKCD
                ...
                Case C_UPDATE       '変更
                    '画面内容をセット
                    If Not f_SetForm_TM_SITEN(pSel_SITEN_BANKCD, pSel_SITENCD) Then
                        Exit Try
                    End If
                    'コントロール使用不可
                    txt_BANK_CD.Enabled = False
                    txt_SITEN_CD.Enabled = False

                    '行遷移
                    lblTotal.Text = paryKEY_8052.Count
...
#Region "cmdSave_Click 保存ボタンクリック処理"
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        Try
            ...
            'frmGJ8050.pSaveSITEN_BANK_CD = txt_BANK_CD.Text.TrimEnd                        '2010/10/16 DEL JBD200
            'frmGJ8050.pSaveSITEN_CD = txt_SITEN_CD.Text.TrimEnd                            '2010/10/16 DEL JBD200
            DirectCast(Owner, frmGJ8050).pSaveFlg = True                                    '2010/10/16 ADD JBD200
            DirectCast(Owner, frmGJ8050).pSaveSITEN_BANK_CD = txt_BANK_CD.Text.TrimEnd      '2010/10/16 ADD JBD200
            DirectCast(Owner, frmGJ8050).pSaveSITEN_CD = txt_SITEN_CD.Text.TrimEnd          '2010/10/16 ADD JBD200
...
#Region "f_RecValidating 行遷移処理"
    ...
    Private Function f_RecValidating(ByVal intRecNo As Integer) As Boolean
        Dim sSITEN_BANKCD As String = String.Empty
        Dim sSITENCD As String = String.Empty
        Dim ret As Boolean = False
        Try
            'KEY取得
            Dim tKEY2 As New frmGJ8050.T_KEY2
            tKEY2 = paryKEY_8052.Item(intRecNo)

            sSITEN_BANKCD = tKEY2.strSITEN_BANK_CD.TrimEnd
            sSITENCD = tKEY2.strSITEN_CD.TrimEnd

            '画面内容をセット
            If Not f_SetForm_TM_SITEN(sSITEN_BANKCD, sSITENCD) Then
                Exit Try
            End If
...
#Region "s_FormControls_TextChanged 画面コントロールTextChangeイベント"
    ...
    Private Sub s_FormControls_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles _
                txt_SITEN_CD.TextChanged, txt_SITEN_KANA.TextChanged, txt_SITEN_NAME.TextChanged, _
                txt_SITEN_NAME.TextChanged, txt_SHITEN_CD.TextChanged
...
#Region "f_Data_Insert データ登録処理"
    ...
    Private Function f_Data_Insert() As Boolean
        ...
        Try
            ...
            '支店マスタデータ存在チェック
            If Not f_SITEN_Exist_Check() Then
                Exit Try
            End If

            '支店マスタ登録
            If Not f_TM_SITEN_Insert() Then
                Exit Try
            End If
    ...
    End Function
#End Region
#Region "f_SITEN_Exist_Check 支店マスタデータ存在チェック"
    '------------------------------------------------------------------
    'プロシージャ名  :f_SITEN_Exist_Check
    '説明            :支店マスタデータ存在チェック
    '引数            :なし
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Public Function f_SITEN_Exist_Check() As Boolean

        Dim sSql As String = String.Empty
        Dim dstDataSet As New DataSet
        Dim strSEIRICD As String = String.Empty
        Dim ret As Boolean = False
        Dim wSITEN_CD As String

        Try
            '支店コード変換
            wSITEN_CD = Format(CInt(txt_SITEN_CD.Text.Trim), "000")
            'データチェック
            sSql = " SELECT " & vbCrLf
            sSql += "  * " & vbCrLf
            sSql += " FROM" & vbCrLf
            sSql += "  TM_SITEN" & vbCrLf
            sSql += " WHERE" & vbCrLf
            sSql += "  BANK_CD = " & txt_BANK_CD.Text.TrimEnd & vbCrLf
            sSql += " AND " & vbCrLf
            sSql += "  SITEN_CD = " & wSITEN_CD & vbCrLf
    ...
    End Function
#End Region
#Region "f_TM_SITEN_Insert 支店マスタデータ登録"
    '------------------------------------------------------------------
    'プロシージャ名  :f_TM_SITEN_Insert
    '説明            :支店マスタデータ登録
    '引数            :1.blnIsTran       Boolean     ﾄﾗﾝｻﾞｸｼｮﾝフラグ
    '                 2.myTrans         Oracle.DataAccess.Client.OracleTransaction
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Private Function f_TM_SITEN_Insert() As Boolean
        ...
        Try
            ...
            '引き渡し
            Dim paraBANK_CD As OracleParameter = Cmd.Parameters.Add("IN_SITEN_BANK_CD", txt_BANK_CD.Text.Trim)              '金融機関コード
            Dim paraSITEN_CD As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_CD", wSITEN_CD)            '支店コード
            Dim paraBANK_KANA As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_KANA", txt_SITEN_KANA.Text.Trim)      '支店(カナ)
            Dim paraBANK_NAME As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_NAME", txt_SITEN_NAME.Text.TrimEnd)   '支店(漢字)
    ...
    End Function
#End Region
#Region "f_SetForm_TM_SITEN 支店マスタデータ画面セット"
    '------------------------------------------------------------------
    'プロシージャ名  :f_SetForm_TM_SITEN
    '説明            :支店マスタデータ画面セット
    '引数            :なし
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Public Function f_SetForm_TM_SITEN(ByVal strBANK_CD As String, ByVal strSITEN_CD As String) As Boolean
        ...
        Try
            ...
            sSql += " AND" & vbCrLf
            sSql += "  SITEN_CD = " & strSITEN_CD.TrimEnd & vbCrLf
...
#Region "f_TM_SITEN_Update 支店マスタデータ更新"
    '------------------------------------------------------------------
    'プロシージャ名  :f_TM_SITEN_Update
    '説明            :支店マスタデータ更新
    '引数            :1.blnIsTran       Boolean     ﾄﾗﾝｻﾞｸｼｮﾝフラグ
    '                 2.myTrans         Oracle.DataAccess.Client.OracleTransaction
    '戻り値          :Boolean(正常True/エラーFalse)
    '------------------------------------------------------------------
    Private Function f_TM_SITEN_Update() As Boolean
        Dim Cmd As New OracleCommand
        Dim ret As Boolean = False
        'Dim intDATAKBN As Integer
        ...
        Try
            ...
            '引き渡し
            Dim paraBANK_CD As OracleParameter = Cmd.Parameters.Add("IN_SITEN_BANK_CD", txt_BANK_CD.Text.Trim)              '金融機関コード
            Dim paraSITEN_CD As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_CD", txt_SITEN_CD.Text.Trim)            '支店コード
            Dim paraBANK_KANA As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_KANA", txt_SITEN_KANA.Text.Trim)      '支店(カナ)
            Dim paraBANK_NAME As OracleParameter = Cmd.Parameters.Add("IN_SITEN_SITEN_NAME", txt_SITEN_NAME.Text.TrimEnd)   '支店(漢字)
            ''If rbtn_DATA_KBN1.Checked = True Then
            ''    intDATAKBN = 0   '有効
    ...
    Private Function f_Input_Check() As Boolean
        ...
        Try
            ...
            If txt_SITEN_CD.Text.TrimEnd = "" Then
                Show_MessageBox_Add("W008", "支店コード")      '@0は必須入力項目です。
                txt_SITEN_CD.Focus()
                Exit Try
            End If
...
#Region "f_SetForm_Data 支店マスタデータ画面セット"
    '------------------------------------------------------------------
    ...
    Public Function f_SetForm_Data(ByVal dstDataSet As DataSet) As Boolean
        ...
        Try
            ...
            With dstDataSet.Tables(0)
                ...
                '支店コード
                txt_SITEN_CD.Text = WordHenkan("N", "S", .Rows(0)("SITEN_CD"))
                '支店名（ｶﾅ）
                txt_SITEN_KANA.Text = WordHenkan("N", "S", .Rows(0)("SITEN_KANA"))
                '支店名（漢字）
                txt_SITEN_NAME.Text = WordHenkan("N", "S", .Rows(0)("SITEN_NAME"))
