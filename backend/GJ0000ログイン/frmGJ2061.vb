...
'*　　①　フォーム名　　　  frmGJ2061.vb
'*
'*　　②　機能概要　　　　　（相殺対応）互助金生産者積立金等返還通知書（全額）発行処理
'*
...
#Region "frmGJ2061_Load Form_Loadイベント"

    '------------------------------------------------------------------
    'プロシージャ名  :frmGJ2061_Load
    '説明            :Form_Loadイベント
    '------------------------------------------------------------------
    Private Sub frmGJ2061_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim ret As Boolean
...
#Region "f_Save 請求入金更新"
    ...
    Private Function f_Save() As Boolean
        ...
        Try
            ...
            '処理区分
            Cmd.CommandText = "PKG_GJ2060.GJ2060_SEIKYU"
            '--------------------
            '請求
            '--------------------
            '期
    ...
    Private Function f_Input_Check(ByRef wSEIKYUSU As Integer) As Boolean
        ...
        Try
            ...
            If wDS.Tables(0).Rows.Count = 0 Then
                Show_MessageBox_Add("W020", "指定された請求（全額返還）処理") '@0がありません。
                Exit Try
            Else
                If WordHenkan("N", "Z", wDS.Tables(0).Rows(0)("KEIYAKU_HENKO_KBN")) <> "0" Then
                    Show_MessageBox_Add("I007", "計算処理で作成された請求（全額返還）処理ではありません。") '@0
                    Exit Try
                End If
            End If
                ...
                If .Rows.Count > 0 Then
                    ...
                    Else
                        If Show_MessageBox_Add("Q012",
                           ...
                           "生産者積立金返還金通知書（全額返還）を" & vbCrLf & "発行してもよろしいですか。") = System.Windows.Forms.DialogResult.No Then
                            Exit Try
                        End If
                    End If
                Else
...
#Region "f_Where_Set 条件セット処理"
    ...
    Private Function f_Where_Set(ByVal wKBN As Integer, ByRef wWHERE As String) As Boolean
        ...
        Try
            ...
            '変更請求区分:4(全額返還のみ対象)
            wWHERE &= "  AND SEIKYU_HENKAN_KBN = 4"
            '契約変更区分:0(新規のみ対象)
