...
import { ContentTypeEnum, EnumServiceResult } from '#/Enums'
...
export class VAxios {
   ...
   */
  uploadPriview(config: AxiosRequestConfig, options?: RequestOptions): Promise<Result> {
    const transform = this.getTransform()
   ...
   * @description:  preview
   */
  preview<T = any>(config: AxiosRequestConfig, options?: RequestOptions): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      this.axiosInstance
        .post<any, AxiosResponse<Result>>(conf.url as string, conf.data, { responseType: 'blob' })
        .then((res: any) => {
          if (res) {
   ...
   * @description:   リクエスト
   */
  request(config: AxiosRequestConfig, options?: RequestOptions): Promise<Result> {
    let conf: AxiosRequestConfig = clone(config, true)
    const transform = this.getTransform()
    const { requestOptions } = this.options
    const opt: RequestOptions = Object.assign({}, requestOptions, options)
            ...
            if (res.headers?.returncode == EnumServiceResult.OK) {
              ...
              if (contentDisposition) {
                fileName = decodeURIComponent(
                  res.headers['content-disposition'].split('=')[2].replace("UTF-8''", '')
                )
              }
