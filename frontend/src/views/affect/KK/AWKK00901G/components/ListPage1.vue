...
<template>
  <a-spin :spinning="loading">
    ...
    <a-card class="mt-2">
      ...
      >
        <vxe-column field="nitteino" title="日程番号" width="100">
          <template #default="{ row }">
            <a @click="goDetail(row)">{{ row.nitteino }}</a>
          </template>
        </vxe-column>
        <vxe-column field="gyomukbnnm" title="業務"></vxe-column>
        <vxe-column field="jigyonm" title="事業名"></vxe-column>
        <vxe-column field="jissinaiyo" title="実施内容"></vxe-column>
        <vxe-column field="jissiyoteiymd" title="実施予定日" formatter="jpDate"></vxe-column>
        <vxe-column field="time" title="時間"></vxe-column>
        <vxe-column field="kaijonm" title="会場"></vxe-column>
        <vxe-column field="kikannm" title="医療機関"></vxe-column>
        <vxe-column field="staffidnms" title="担当者"></vxe-column>
        <vxe-column field="teiin" title="定員"></vxe-column>
      </vxe-table>
    </a-card>
  </a-spin>
</template>
...
import { InitListResponse, NitteiRowVM, SearchNitteiListRequest } from '../type'
...
const emit = defineEmits(['goGroup'])
//--------------------------------------------------------------------------
//データ定義
//--------------------------------------------------------------------------
...
onBeforeRouteUpdate((to, from) => {
  if (to.query.refresh && from.query.nitteino) {
...
//画面遷移
const goDetail = ({ nitteino, iscopy = false }) => {
  router.push({
